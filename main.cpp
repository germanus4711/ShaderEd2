...
int main(int argc, char* argv[])
{
	...
	// Try to initialize audio separately
	// TODO Fails on Mac vvv
	if (SDL_InitSubSystem(SDL_INIT_AUDIO) < 0) {
		ed::Logger::Get().Log("Warning: Failed to initialize SDL2 audio: " + std::string(SDL_GetError()), true);
		// Continue execution anyway
	} else
		ed::Logger::Get().Log("Initialized SDL2 audio");
	...
	if (preload.is_open()) {
		...
		if (wndWidth > desk.w)
			wndWidth = static_cast<short>(desk.w);
		if (wndHeight > desk.h)
			wndHeight = static_cast<short>(desk.h);
	} else {
		ed::Logger::Get().Log("File data/preload.dat doesn\'t exist", true);
	...
	if (coptsParser.WindowWidth > 0)
		wndWidth = static_cast<short>(coptsParser.WindowWidth);
	if (coptsParser.WindowHeight > 0)
		wndHeight = static_cast<short>(coptsParser.WindowHeight);
	...
	// open window
	SDL_Window* wnd = SDL_CreateWindow("SHADERed", (wndPosX == -1) ? SDL_WINDOWPOS_CENTERED : (int)(wndPosX), (wndPosY == -1) ? SDL_WINDOWPOS_CENTERED : (int)(wndPosY), wndWidth, wndHeight, windowFlags);
	SetDpiAware();
	...
	if (fsError)
		ed::Logger::Get().Log("A filesystem error has occurred: " + fsError.message(), true);
	...
	while (engine.Interface().Run) {
			...
			if (event.type == SDL_QUIT) {
				...
				if (event.window.event == SDL_WINDOWEVENT_MOVED || event.window.event == SDL_WINDOWEVENT_MAXIMIZED || event.window.event == SDL_WINDOWEVENT_RESIZED) {
					...
					if (!maximized) {
						...
						wndWidth = (short)tempX;
						wndHeight = (short)tempY;
					}
		...
		// TODO FIX:UNSUPPORTED (log once): POSSIBLE ISSUE: unit 0 GLD_TEXTURE_INDEX_2D is unloadable and bound to sampler type (Float) - using zero texture because texture unloadable
		engine.Render();
	...
	if (!coptsParser.Render && !coptsParser.ConvertCPP) {
		...
		save.write(converter.data, 2);
		save.put((char)fullscreen);
		save.put((char)maximized);
		save.put((char)engine.UI().IsPerformanceMode());
		save.write(converter.data, 2);
		save.close();
	}
