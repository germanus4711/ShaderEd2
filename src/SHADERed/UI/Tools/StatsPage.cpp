...
namespace ed {
	...
	void StatsPage::Refresh(PipelineItem* item, ShaderStage stage)
	{
		...
		if (item->Type == PipelineItem::ItemType::ShaderPass) {
			const auto* pass = static_cast<pipe::ShaderPass*>(item->Data);
			...
			const auto* pass = static_cast<pipe::ComputePass*>(item->Data);

			m_spv = pass->SPV;
		} else if (item->Type == PipelineItem::ItemType::PluginItem) {
			const auto* pass = static_cast<pipe::PluginItemData*>(item->Data);
