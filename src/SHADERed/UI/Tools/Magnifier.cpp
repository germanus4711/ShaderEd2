...
namespace ed {
	...
	Magnifier::Magnifier()
	{
		...
		if (!ShaderInitialized) {
			Shader = gl::CreateShader(&UI_VS_CODE, &UI_PS_CODE, "magnifier");
	...
	{
		if (m_dragging) {
			const float mpy = 1 - m_mousePos.y;
			const float mpx = m_mousePos.x;

			const float moveY = (mpy - m_zoomDrag.y) * m_size.x;
			const float moveX = (m_zoomDrag.x - mpx) * m_size.y;
	...
	void Magnifier::EndMouseAction()
	{
		if (m_selecting && m_mousePos.x != m_posStart.x && m_mousePos.y != 1 - m_posStart.y) {
			...
			if (m_size.x < (25.0f / static_cast<float>(m_w)) * oldSize.x && m_size.y < (25.0f / static_cast<float>(m_h)) * oldSize.y)
				m_size = oldSize;

			m_size.x = std::max<float>(std::min<float>(m_size.x, 1.0f), 25.0f / static_cast<float>(m_w));
			m_size.y = std::max<float>(std::min<float>(m_size.y, 1.0f), 25.0f / static_cast<float>(m_w));
	...
	void Magnifier::Zoom(const float w, const bool mouseAsPosition)
	{
		float oldZoomWidth = m_size.x, oldZoomHeight = m_size.y;

		m_size.x = std::max<float>(std::min<float>(m_size.x * w, 1.0f), 25.0f / static_cast<float>(m_w));
		m_size.y = std::max<float>(std::min<float>(m_size.y * w, 1.0f), 25.0f / static_cast<float>(m_w));
	...
	void Magnifier::Render() const
	{
		...
		const float mpy = 1 - m_mousePos.y;
		const float mpx = m_mousePos.x;
		const float zx = m_pos.x + m_size.x * m_posStart.x;
		const float zy = 1 - (m_pos.y + m_size.y * (1 - m_posStart.y));
		const float zw = (mpx - m_posStart.x) * m_size.x;
		const float zh = (mpy - m_posStart.y) * m_size.y;

		const glm::mat4 zMatWorld = glm::translate(glm::mat4(1), glm::vec3(zx * static_cast<float>(m_w), zy * static_cast<float>(m_h), 0.0f))
			* glm::scale(glm::mat4(1), glm::vec3(zw, zh, 1.0f));

		const glm::mat4 zMatProj = glm::ortho(0.0f, static_cast<float>(m_w), static_cast<float>(m_h), 0.0f, 0.1f, 100.0f);

		glUniformMatrix4fv(static_cast<int>(m_uMatWVPLoc), 1, GL_FALSE, glm::value_ptr(zMatProj * zMatWorld));
