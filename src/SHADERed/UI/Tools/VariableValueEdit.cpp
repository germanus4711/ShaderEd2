...
namespace ed {
	...
	bool VariableValueEditUI::Update()
	{
		...
		if (ImGui::Button(UI_ICON_FX, ImVec2(25, 0))) {
			// find the first function that is compatible with this value type
			for (int i = 0; i < static_cast<int>(FunctionShaderVariable::Count); i++) {
				if (m_var->Function == FunctionShaderVariable::PluginFunction)
					break;

				if (FunctionVariableManager::HasValidReturnType(m_var->GetType(), static_cast<FunctionShaderVariable>(i))) {
					FunctionVariableManager::AllocateArgumentSpace(m_var, static_cast<FunctionShaderVariable>(i));
					break;
				}
			}
		}
	...
	bool VariableValueEditUI::m_drawRegular() const
	{
	...
	bool VariableValueEditUI::m_drawFunction() const
	{
		bool ret = false;
		...
		ImGui::SameLine();
		ImGui::PushItemWidth(-1);
		const char* inputComboPreview = m_var->Function == FunctionShaderVariable::PluginFunction ? m_var->PluginFuncData.Name : FUNCTION_NAMES[static_cast<int>(m_var->Function)];
		if (ImGui::BeginCombo(("##func" + std::string(m_var->Name)).c_str(), inputComboPreview)) {
			for (int n = 1; n < static_cast<int>(FunctionShaderVariable::Count); n++) {
				bool is_selected = (n == static_cast<int>(m_var->Function));
				if (n != static_cast<int>(FunctionShaderVariable::PluginFunction)) {
					if (FunctionVariableManager::HasValidReturnType(m_var->GetType(), static_cast<FunctionShaderVariable>(n)) && ImGui::Selectable(FUNCTION_NAMES[n], is_selected)) {
						FunctionVariableManager::AllocateArgumentSpace(m_var, static_cast<FunctionShaderVariable>(n));
						ret = true;
					}
				} else {
					...
					if (modified) {
						...
						m_var->Arguments = static_cast<char*>(malloc(m_var->PluginFuncData.Owner->VariableFunctions_GetArgsSize(m_var->PluginFuncData.Name, (plugin::VariableType)m_var->GetType())));
		...
		case FunctionShaderVariable::Pointer: {
			...
			if (ImGui::BeginCombo(("##ptrVars" + std::string(m_var->Name)).c_str(), m_var->Arguments)) {
				for (auto& n : varList) {
					if (m_var == n || n == nullptr)
						break;

					if (m_var->GetType() != n->GetType())
						continue;

					bool is_selected = strcmp(m_var->Arguments, n->Name) == 0;
					if (ImGui::Selectable(n->Name, is_selected)) {
						strcpy(m_var->Arguments, n->Name);
						ret = true;
					}
					if (is_selected)
						ImGui::SetItemDefaultFocus();
		...
		case FunctionShaderVariable::CameraSnapshot: {
			...
			if (ImGui::BeginCombo(("##camSnaps" + std::string(m_var->Name)).c_str(), m_var->Arguments)) {
				for (const auto& n : camList) {
					const bool is_selected = strcmp(m_var->Arguments, n.c_str()) == 0;
					if (ImGui::Selectable(n.c_str(), is_selected)) {
						strcpy(m_var->Arguments, n.c_str());
						ret = true;
					}
					if (is_selected)
						ImGui::SetItemDefaultFocus();
		...
		case FunctionShaderVariable::ObjectProperty: {
			...
			if (ImGui::BeginCombo(("##pipeItems" + std::string(m_var->Name)).c_str(), m_var->Arguments)) {
				for (auto& n : pipeList) {
					if (n->Type == PipelineItem::ItemType::ShaderPass) {
						auto* pass = static_cast<pipe::ShaderPass*>(n->Data);
						for (auto item : pass->Items) {
							...
							const bool is_selected = strcmp(m_var->Arguments, item->Name) == 0;
