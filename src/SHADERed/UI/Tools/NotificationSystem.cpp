...
namespace ed {
	...
	{
		m_notifs.emplace_back(id, text, buttonText, fn, owner);
	}
	void NotificationSystem::Render()
	{
		...
		const ImVec4 textClr = ImGui::GetStyleColorVec4(ImGuiCol_Text);
		const ImVec4 windowClr = ImGui::GetStyleColorVec4(ImGuiCol_WindowBg);
		...
		for (int i = 0; i < m_notifs.size(); i++) {
			bool stayOpen = true;
			auto window_pos = ImVec2(io.DisplaySize.x - DISTANCE, io.DisplaySize.y - DISTANCE - yOffset);
			...
			if (ImGui::Begin(("##updateNotification" + std::to_string(i)).c_str(), nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav)) {
				...
				if (m_notifs[i].Handler && !m_notifs[i].ButtonText.empty()) {
