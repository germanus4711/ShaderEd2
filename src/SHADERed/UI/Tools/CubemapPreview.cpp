...
namespace ed {
	...
	void CubemapPreview::Init(const int w, const int h)
	{
		...
		m_cubeShader = gl::CreateShader(&CUBEMAP_VS_CODE, &CUBEMAP_PS_CODE, "cubemap projection");
		...
		m_fsVAO = eng::GeometryFactory::CreatePlane(m_fsVBO, static_cast<float>(w), static_cast<float>(h), gl::CreateDefaultInputLayout());
		m_cubeFBO = gl::CreateSimpleFramebuffer(w, h, m_cubeTex, m_cubeDepth);

		if (m_cubeFBO == 0)
			Logger::Get().Log("Failed to create cubemap preview texture", true);
	}
	void CubemapPreview::Draw(const GLuint tex) const
	{
		...
		glUniformMatrix4fv(static_cast<int>(m_uMatWVPLoc), 1, GL_FALSE, glm::value_ptr(matVP * matW));
