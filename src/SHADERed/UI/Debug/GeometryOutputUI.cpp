...
namespace ed {
	...
	void DebugGeometryOutputUI::m_buildVertexBuffers()
	{
		...
		if (px) {
			...
			for (auto& prim : px->GeometryOutput)
				for (auto& v : prim.Position) {
					m_indexPos.emplace_back(0.0f, 0.0f);
					m_indexPos.back().x = (v.x / v.w + 1.0f) * 0.5f;
					m_indexPos.back().y = 1.0f - (v.y / v.w + 1.0f) * 0.5f;
				}
		}
	...
	void DebugGeometryOutputUI::m_render()
	{
		...
		if (m_is3D) {
			...
			glUniformMatrix4fv(static_cast<GLint>(m_uMatLoc), 1, GL_FALSE, glm::value_ptr(matVP));
		} else
			glUniformMatrix4fv(static_cast<GLint>(m_uMatLoc), 1, GL_FALSE, glm::value_ptr(glm::mat4(1.0f)));
	...
	void DebugGeometryOutputUI::Update(float delta)
	{
		...
		if (m_lastFBOHeight != wndHeight || m_lastFBOWidth != wndWidth) {
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, static_cast<GLsizei>(wndWidth), static_cast<GLsizei>(wndHeight), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
			glBindTexture(GL_TEXTURE_2D, m_fboDepth);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH24_STENCIL8, static_cast<GLsizei>(wndWidth), static_cast<GLsizei>(wndHeight), 0, GL_DEPTH_STENCIL, GL_UNSIGNED_INT_24_8, nullptr);
		...
		if (px != nullptr && m_is3D == 0) {
			...
			if (px->RenderTexture != nullptr)
				ImGui::Image(reinterpret_cast<ImTextureID>(static_cast<uintptr_t>(px->RenderTexture->Texture)), ImVec2(m_lastFBOWidth, m_lastFBOHeight), ImVec2(0, 1), ImVec2(1, 0), ImVec4(0.6f, 0.6f, 0.6f, 1.0f));
			else
				ImGui::Image(reinterpret_cast<ImTextureID>(static_cast<uintptr_t>(m_data->Renderer.GetTexture())), ImVec2(m_lastFBOWidth, m_lastFBOHeight), ImVec2(0, 1), ImVec2(1, 0), ImVec4(0.6f, 0.6f, 0.6f, 1.0f));
		...
		ImGui::Image(reinterpret_cast<ImTextureID>(static_cast<uintptr_t>(m_fboColor)), ImVec2(m_lastFBOWidth, m_lastFBOHeight), ImVec2(0, 1), ImVec2(1, 0), ImVec4(0.9f, 0.9f, 0.9f, 0.6f));
		// camera controlls
