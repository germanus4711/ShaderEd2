...
namespace ed {
	auto HistoryCallback(ImGuiTextEditCallbackData* data) -> int
	{
		auto* history = static_cast<HistoryData*>(data->UserData);

		if (history->Index >= static_cast<int>(history->Data.size()) || history->Data.empty()) {
			history->Index = -1;
			return 0;
		}
		...
		data->BufTextLen = static_cast<int>(history->Data[history->Data.size() - history->Index - 1].size());
	...
	void DebugImmediateUI::Update(float delta)
	{
		...
		clipper.Begin(static_cast<int>(m_lineOffsets.size()));
		while (clipper.Step()) {
			...
			if (inputStr == "? clear") { // this is just a debug thingy, maybe support actual commands?
				...
				if (res == nullptr || resType == nullptr) {
					...
					if (resType->value_type == spvm_value_type_vector && m_data->Debugger.GetVMImmediate()->results[resType->pointer].value_type == spvm_value_type_float) {
						if (resType->member_count == 3 or resType->member_count == 4)
