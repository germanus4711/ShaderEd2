...
namespace ed {
	...
	void PropertyUI::Update(float delta)
	{
		if (m_current != nullptr || m_currentObj != nullptr) {
			...
			if (m_current != nullptr) {
				ImGui::Text("%s", m_current->Name);
				ImGui::Text("%s", PIPELINE_ITEM_NAMES[static_cast<int>(m_current->Type)]);
				if (m_current->Type == PipelineItem::ItemType::Geometry) {
					ImGui::SameLine();
					ImGui::Text("(%s)", GEOMETRY_NAMES[static_cast<int>(static_cast<pipe::GeometryItem*>(m_current->Data)->Type)]);
				} else if (m_current->Type == PipelineItem::ItemType::PluginItem) {
					ImGui::SameLine();
					ImGui::Text("(%s)", static_cast<pipe::PluginItemData*>(m_current->Data)->Type);
				}
				...
				if (IsRenderTexture())
					...
					ImGui::Text("%s", m_currentObj->Plugin->Type);
				else
				...
				if (ImGui::Button("Ok##pui_itemname", ImVec2(-1, 0))) {
					if (m_data->Pipeline.Has(m_itemName))
						...
						if (m_current->Type == PipelineItem::ItemType::PluginItem) {
							auto pdata = static_cast<pipe::PluginItemData*>(m_current->Data);
			...
			if (m_current != nullptr) {
				if (m_current->Type == ed::PipelineItem::ItemType::ShaderPass) {
					auto item = reinterpret_cast<ed::pipe::ShaderPass*>(m_current->Data);
					...
					for (item->RTCount = 0; item->RTCount < MAX_RENDER_TEXTURES; item->RTCount++) {
						int i = static_cast<unsigned char>(item->RTCount);
						...
						if (ImGui::BeginCombo("##pui_rt_combo", name.c_str())) {
							...
							for (unsigned int k : item->RenderTextures) {
								if (k == 0)
									continue;

								if (!windowAlreadyBound && k == m_data->Renderer.GetTexture()) {
									windowAlreadyBound = true;
									break;
								}

								for (int j = 0; j < rts.size(); j++) {
									if (rts[j]->Type != ObjectType::RenderTexture || (rts[j]->Type == ObjectType::RenderTexture && k == rts[j]->Texture)) {
							...
							// prevent duplicates
							for (unsigned int RenderTexture : item->RenderTextures) {
								if (RenderTexture == 0)
									break;

								std::string _name = RenderTexture == m_data->Renderer.GetTexture() ? "Window" : m_data->Objects.GetByTextureID(RenderTexture)->Name;
							...
							for (auto& rt : rts)
								if (rt->Type == ObjectType::RenderTexture) {
									GLuint texID = rt->Texture;
									if (ImGui::Selectable(rt->Name.c_str(), rtID == texID)) {
										m_data->Parser.ModifyProject();
					...
					auto* item = reinterpret_cast<ed::pipe::ComputePass*>(m_current->Data);
					...
					auto* item = static_cast<ed::pipe::AudioPass*>(m_current->Data);
					...
					auto* item = static_cast<ed::pipe::GeometryItem*>(m_current->Data);
					...
					if (ImGui::BeginCombo("##pui_geo_instancebuf", ((item->InstanceBuffer == nullptr) ? "NULL" : (m_data->Objects.GetByBufferID(((BufferObject*)item->InstanceBuffer)->ID)->Name.c_str())))) {
						...
						for (auto i : bufList) {
							if (i->Type != ObjectType::Buffer)
								continue;

							BufferObject* buf = i->Buffer;

							if (ImGui::Selectable(i->Name.c_str(), buf == item->InstanceBuffer)) {
					...
					auto* data = static_cast<pipe::RenderState*>(m_current->Data);
					...
					auto* item = static_cast<ed::pipe::Model*>(m_current->Data);
					...
					const auto& bufList = m_data->Objects.GetObjects();
					ImGui::PushItemWidth(-1);
					if (ImGui::BeginCombo("##pui_mdl_instancebuf", ((item->InstanceBuffer == nullptr) ? "NULL" : (m_data->Objects.GetByBufferID(static_cast<BufferObject*>(item->InstanceBuffer)->ID)->Name.c_str())))) {
						// null element
						if (ImGui::Selectable("NULL", item->InstanceBuffer == nullptr)) {
							item->InstanceBuffer = nullptr;
						...
						for (auto i : bufList) {
							...
							if (ImGui::Selectable(i->Name.c_str(), buf == item->InstanceBuffer)) {
								...
								char* owner = m_data->Pipeline.GetItemOwner(m_current->Name);
								auto* ownerData = (pipe::ShaderPass*)(m_data->Pipeline.Get(owner)->Data);
					...
					auto* item = static_cast<ed::pipe::VertexBuffer*>(m_current->Data);
					...
					ImGui::PushItemWidth(-1);
					if (ImGui::BeginCombo("##pui_vb_buffer", ((item->Buffer == nullptr) ? "NULL" : (m_data->Objects.GetByBufferID(static_cast<BufferObject*>(item->Buffer)->ID)->Name.c_str())))) {
						// null element
						...
						for (auto& i : bufList) {
							...
							ed::BufferObject* buf = i->Buffer;

							if (ImGui::Selectable(i->Name.c_str(), buf == item->Buffer)) {
								item->Buffer = buf;
								auto fmtList = m_data->Objects.ParseBufferFormat(buf->ViewFormat);
					...
					if (ImGui::BeginCombo("##pui_geo_instancebuf", ((item->InstanceBuffer == nullptr) ? "NULL" : (m_data->Objects.GetByBufferID(((BufferObject*)item->InstanceBuffer)->ID)->Name.c_str())))) {
						...
						if (ImGui::Selectable("NULL", item->InstanceBuffer == nullptr)) {
							...
							auto* bobj = static_cast<BufferObject*>(item->Buffer);
				...
				if (ImGui::DragFloat2("##prop_rt_rsize", glm::value_ptr(m_currentRT->RatioSize), 0.01f)) {
					...
					glm::ivec2 newSize(m_currentRT->RatioSize.x * static_cast<float>(m_data->Renderer.GetLastRenderSize().x),
						m_currentRT->RatioSize.y * static_cast<float>(m_data->Renderer.GetLastRenderSize().y));
				...
				if (ImGui::BeginCombo("##pui_texture_combo", m_currentImg->DataPath[0] == 0 ? "CLEAR" : m_currentImg->DataPath)) {
					...
					for (auto i : texData)
						if (i->Type == ObjectType::Texture && ImGui::Selectable(i->Name.c_str(), i->Name == m_currentImg->DataPath))
							strcpy(m_currentImg->DataPath, i->Name.c_str());
	...
	void PropertyUI::Open(ed::PipelineItem* item)
	{
		if (item != nullptr) {
			...
			if (item->Type == PipelineItem::ItemType::ComputePass) {
				const auto cPass = static_cast<pipe::ComputePass*>(item->Data);
	...
	void PropertyUI::Open(ObjectManagerItem* obj)
	{
		...
		memset(m_itemName, 0, std::size(m_itemName));
