	...
	void ObjectPreviewUI::OnEvent(const SDL_Event& e)
	{
		if (m_curHoveredItem != -1) {
			if (e.type == SDL_MOUSEBUTTONDOWN) {
				...
				if (bool isAltDown = keyState[SDL_SCANCODE_LALT] || keyState[SDL_SCANCODE_RALT]) {
		...
		for (int i = 0; i < m_items.size(); i++) {
			...
			if (ImGui::Begin((name + "###objprev" + std::to_string(i)).c_str(), (bool*)&m_isOpen[i])) {
				...
				if (item->Plugin != nullptr) {
					...
					float scale = std::min<float>(aSize.x / (float)iSize.x, aSize.y / (float)iSize.y);
					aSize.x = (float)iSize.x * scale;
					aSize.y = (float)iSize.y * scale;
					...
					if (item->Type == ObjectType::CubeMap) {
						...
						ImGui::Image(reinterpret_cast<void*>(static_cast<intptr_t>(m_cubePrev.GetTexture())), aSize, ImVec2(zPos.x, zPos.y + zSize.y), ImVec2(zPos.x + zSize.x, zPos.y));
						...
						ImGui::SetCursorPosX(posX);
						ImGui::SetCursorPosY(posY);
						ImGui::Image(reinterpret_cast<void*>(static_cast<intptr_t>(m_zoomColor[i])), aSize, ImVec2(zPos.x, zPos.y + zSize.y), ImVec2(zPos.x + zSize.x, zPos.y));
						...
						const glm::vec2& zPos = m_zoom[i].GetZoomPosition();
						const glm::vec2& zSize = m_zoom[i].GetZoomSize();
						m_tex3DPrev.Draw(item->Texture, objSize.x, objSize.y, ((float)m_cachedImgSlice[i] + 0.5f) / (float)objSize.z);
						ImGui::Image(reinterpret_cast<void*>(static_cast<intptr_t>(m_tex3DPrev.GetTexture())), aSize, ImVec2(zPos.x, zPos.y), ImVec2(zPos.x + zSize.x, zPos.y + zSize.y));
						...
						ImGui::SetCursorPosX(posX);
						ImGui::SetCursorPosY(posY);
						ImGui::Image(reinterpret_cast<void*>(static_cast<intptr_t>(m_zoomColor[i])), aSize, ImVec2(zPos.x, zPos.y), ImVec2(zPos.x + zSize.x, zPos.y + zSize.y));

						// slices
						posSize = ImGui::GetWindowContentRegionMax();
						ImGui::SetCursorPosY(posSize.y - Settings::Instance().CalculateSize((float)Settings::Instance().General.FontSize + 5.0f));
						...
						ImGui::PlotHistogram("Samples", m_samples, IM_ARRAYSIZE(m_samples), 0, nullptr, 0.0f, 1.0f, ImVec2(0, 80));
						...
						for (auto& j : m_cachedBufFormat[i])
							perRow += ShaderVariable::GetSize(j, true);
									...
									for (auto& pipeItem : m_data->Pipeline.GetList()) {
										if (m_data->Objects.IsUniformBound(item, pipeItem) != -1) {
											if (pipeItem->Type == PipelineItem::ItemType::ComputePass) {
												...
												suggestion.Thread = glm::ivec3(s.x * (float)image->Size.x, s.y * (float)image->Size.y, 0);
						...
						if (item->Type == ObjectType::RenderTexture) {
							...
							// debugger overlay
							if (!pixelList.empty() && (Settings::Instance().Debug.PixelOutline || Settings::Instance().Debug.PrimitiveOutline)) {
