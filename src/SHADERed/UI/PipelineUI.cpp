...
namespace ed {
	...
	void PipelineUI::Update(float delta)
	{
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (items[i]->Type == PipelineItem::ItemType::ShaderPass) {
				...
				auto* data = static_cast<ed::pipe::ShaderPass*>(items[i]->Data);

				bool showItems = true;
				for (auto& j : m_expandList)
					if (j == data) {
						showItems = false;
						break;
					}
		...
		if (ImGui::BeginPopupModal("Debug compute shader")) {
			...
			const int localInvocationIndex = static_cast<int>((m_thread[2] % m_localSizeZ) * m_localSizeX * m_localSizeZ + (m_thread[1] % m_localSizeY) * m_localSizeX + (m_thread[0] % m_localSizeX));
			...
			if (ImGui::Button("Start")) {
				...
				m_data->Debugger.PrepareComputeShader(m_modalItem, static_cast<int>(m_thread[0]), static_cast<int>(m_thread[1]), static_cast<int>(m_thread[2]));
				dynamic_cast<PixelInspectUI*>(m_ui->Get(ViewID::PixelInspect))->StartDebugging(editor, pluginEditor, nullptr);
	...
	void PipelineUI::DeleteItem(PipelineItem* item) const
	{
		...
		if (item->Type == PipelineItem::ItemType::ShaderPass) {
			...
			auto* pass = static_cast<pipe::PluginItemData*>(item->Data);
			for (const auto& child : pass->Items) {
				if (props->HasItemSelected() && props->CurrentItemName() == child->Name)
					props->Close();
				if (prev->IsPicked(child))
		...
		// tell the pipeline to remove this item
		m_data->Messages.ClearGroup(item->Name);
		m_data->Renderer.RemoveItemVariableValues(item);
		m_data->Pipeline.Remove(item->Name);
	}

	void PipelineUI::m_renderItemUpDown(pipe::PluginItemData* owner, std::vector<ed::PipelineItem*>& items, int index) const
	{
		...
		if (ImGui::Button(std::string(UI_ICON_ARROW_UP "##U" + std::string(items[index]->Name)).c_str(), BUTTON_ICON_SIZE)) {
			if (index != 0) {
				...
				const auto props = (reinterpret_cast<PropertyUI*>(m_ui->Get(ViewID::Properties)));
	...
	bool PipelineUI::m_renderItemContext(std::vector<ed::PipelineItem*>& items, int index)
	{
		...
		if (ImGui::BeginPopupContextItem(("##context_" + std::string(items[index]->Name)).c_str())) {
			...
			auto* pldata = static_cast<pipe::PluginItemData*>(items[index]->Data);
			...
			if (items[index]->Type == PipelineItem::ItemType::ShaderPass || items[index]->Type == PipelineItem::ItemType::ComputePass || items[index]->Type == PipelineItem::ItemType::AudioPass || hasPluginAddMenu) {
				...
				if ((hasPluginAddMenu || items[index]->Type == PipelineItem::ItemType::ShaderPass) && ImGui::BeginMenu("Add")) {
					...
					if (!isPlugin)
						...
						auto piData = static_cast<pipe::PluginItemData*>(items[index]->Data);
				...
				if ((hasPluginShaders || !isPlugin) && ImGui::BeginMenu("Edit Code")) {
					...
					if (items[index]->Type == PipelineItem::ItemType::ShaderPass) {
						auto* passData = static_cast<pipe::ShaderPass*>(items[index]->Data);
						...
						auto* passData = static_cast<pipe::ComputePass*>(items[index]->Data);
						...
						auto* passData = static_cast<pipe::AudioPass*>(items[index]->Data);
				...
				if (items[index]->Type == PipelineItem::ItemType::ComputePass && ImGui::MenuItem("Debug")) {
					auto* pass = static_cast<pipe::ComputePass*>(items[index]->Data);

					m_isComputeDebugOpen = true;
					m_modalItem = items[index];

					m_localSizeX = m_localSizeY = m_localSizeZ = 1;
					m_groupsX = static_cast<int>(pass->WorkX);
					m_groupsY = static_cast<int>(pass->WorkY);
					m_groupsZ = static_cast<int>(pass->WorkZ);
	...
	void PipelineUI::m_renderVarFlags(ed::ShaderVariable* var, char flags) const
	{
		...
		var->Flags = 0;
		if (isInvert)
			var->Flags |= static_cast<char>(ShaderVariable::Flag::Inverse);
		if (isLastFrame)
			var->Flags |= static_cast<char>(ShaderVariable::Flag::LastFrame);
	}
	void PipelineUI::m_renderInputLayoutManagerUI() const
	{
		static auto iValueType = InputLayoutValue::Position;
		static char semanticName[32];

		auto itemData = static_cast<pipe::ShaderPass*>(m_modalItem->Data);
		...
		for (auto& el : els) {
			...
			if (ImGui::Button(UI_ICON_ARROW_UP, BUTTON_ICON_SIZE) && id != 0) {
				// TODO Schoene Variante :-) els[e] <-?
				const InputLayoutItem temp = els[id - 1];
				els[id - 1] = el;
				els[id] = temp;
		...
		/* TYPE */
		ImGui::PushItemWidth(-ImGui::GetStyle().FramePadding.x);
		if (ImGui::Combo("##inputType", reinterpret_cast<int*>(&iValueType), ATTRIBUTE_VALUE_NAMES, std::size(ATTRIBUTE_VALUE_NAMES))) {
			/* Nothing to do here? */
		}
		ImGui::NextColumn();
	...
	void PipelineUI::m_renderVariableManagerUI()
	{
		...
		const ShaderLanguage vsLang = ShaderCompiler::GetShaderLanguageFromExtension(static_cast<pipe::ShaderPass*>(itemData)->VSPath);
		const bool isGLSL = (vsLang == ShaderLanguage::GLSL) || (vsLang == ShaderLanguage::VulkanGLSL);
		...
		int id = 0;
		std::vector<ed::ShaderVariable*>& els = isCompute ? ((pipe::ComputePass*)itemData)->Variables.GetVariables() : (isAudio ? static_cast<pipe::AudioPass*>(itemData)->Variables.GetVariables() : static_cast<pipe::ShaderPass*>(itemData)->Variables.GetVariables());
		...
		for (auto& el : els) {
			...
			if (ImGui::Button(UI_ICON_ARROW_UP, BUTTON_ICON_SIZE) && id != 0) {
				...
				auto* pinState = dynamic_cast<PinnedUI*>(m_ui->Get(ViewID::Pinned));
				const bool containsCur = pinState->Contains(el->Name);
				const bool containsDown = pinState->Contains(els[id - 1]->Name);
			...
			/* DELETE BUTTON */
			if (ImGui::Button(UI_ICON_DELETE, BUTTON_ICON_SIZE)) {
				dynamic_cast<PinnedUI*>(m_ui->Get(ViewID::Pinned))->Remove(el->Name); // unpin if pinned

				m_data->Parser.ModifyProject();

				if (isCompute)
					static_cast<pipe::ComputePass*>(itemData)->Variables.Remove(el->Name);
				else if (isAudio)
					static_cast<pipe::AudioPass*>(itemData)->Variables.Remove(el->Name);
				else {
					...
					static_cast<pipe::ShaderPass*>(itemData)->Variables.Remove(el->Name);
				}
			...
			if (ImGui::Combo("##inputType", reinterpret_cast<int*>(&tempType), isGLSL ? VARIABLE_TYPE_NAMES_GLSL : VARIABLE_TYPE_NAMES, std::size(VARIABLE_TYPE_NAMES)))
			...
			/* SYSTEM VALUE */
			ImGui::PushItemWidth(-ImGui::GetStyle().FramePadding.x);
			const char* systemComboPreview = el->System == SystemShaderVariable::PluginVariable ? (el->PluginSystemVarData.Name) : (SYSTEM_VARIABLE_NAMES[static_cast<int>(el->System)]);

			if (ImGui::BeginCombo("##system", systemComboPreview)) {
				bool removeFromPins = false;
				for (int n = 0; n < std::size(SYSTEM_VARIABLE_NAMES); n++) {
					const bool is_selected = (n == (int)el->System);
					if (n != static_cast<int>(SystemShaderVariable::PluginVariable)) {
						if ((n == 0 || SystemVariableManager::GetType(static_cast<SystemShaderVariable>(n)) == el->GetType())
							&& ImGui::Selectable(SYSTEM_VARIABLE_NAMES[n], is_selected)) {
							el->System = static_cast<SystemShaderVariable>(n);
							m_data->Parser.ModifyProject();
							removeFromPins = true;
						}
					} else {
		...
		const char* inputComboPreview = iVariable.System == SystemShaderVariable::PluginVariable ?
			...
			SYSTEM_VARIABLE_NAMES[static_cast<int>(iVariable.System)];
		if (ImGui::BeginCombo("##createVarsystem", inputComboPreview)) {
			for (int n = 0; n < static_cast<int>(SystemShaderVariable::Count); n++) {
				const bool is_selected = (n == static_cast<int>(iVariable.System));
				if (n != static_cast<int>(SystemShaderVariable::PluginVariable)) {
					if ((n == 0 || ed::SystemVariableManager::GetType(static_cast<SystemShaderVariable>(n)) == iVariable.GetType())
						&& ImGui::Selectable(SYSTEM_VARIABLE_NAMES[n], is_selected))
						iVariable.System = static_cast<SystemShaderVariable>(n);
				} else {
					if (m_data->Plugins.ShowSystemVariables(&iVariable.PluginSystemVarData, iVariable.GetType()))
						iVariable.System = SystemShaderVariable::PluginVariable;
				}
	...
	{
		static bool scrollToBottom = false;
		// static int shaderTypeSel = 0;
		static int shaderVarSel = 0;
		...
		// render the value edit window if needed
		...
		ImGui::PushItemWidth(-ImGui::GetStyle().FramePadding.x);
		shaderVarSel = std::min<int>(shaderVarSel, static_cast<int>(vars.size()) - 1);
		if (const char* inputComboPreview = !vars.empty() ?
				vars[shaderVarSel]->Name :
				"-- NONE --";
			ImGui::BeginCombo("##itemvarname", inputComboPreview)) {
			for (int n = 0; n < vars.size(); n++) {
				const bool is_selected = (n == shaderVarSel);
		...
		ImGui::PushItemWidth(-ImGui::GetStyle().FramePadding.x);
		if (ImGui::Button(UI_ICON_ADD, BUTTON_ICON_SIZE) && !vars.empty()) {
			bool alreadyAdded = false;
			for (const auto& allItem : allItems)
				if (strcmp(vars[shaderVarSel]->Name, allItem.Variable->Name) == 0 && allItem.Item == m_modalItem) {
					alreadyAdded = true;
					break;
				}
	...
	void PipelineUI::m_renderResourceManagerUI() const
	{
	...
	{
		auto* data = static_cast<ed::pipe::ShaderPass*>(item->Data);

		std::string expandTxt = UI_ICON_UP;
		for (auto& i : m_expandList)
			if (i == data) {
		...
		if (ImGui::Selectable(item->Name, false, ImGuiSelectableFlags_AllowDoubleClick))
			if (ImGui::IsMouseDoubleClicked(0)) {
				if (Settings::Instance().General.OpenShadersOnDblClk) {
					if (Settings::Instance().General.UseExternalEditor && m_data->Parser.GetOpenedFile().empty())
				...
				if (Settings::Instance().General.ItemPropsOnDblCLk) {
					auto* props = reinterpret_cast<PropertyUI*>(m_ui->Get(ViewID::Properties));
					props->Open(item);
				}
			}
		...
		if (ImGui::BeginDragDropTarget()) {
			...
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("PipelineItemPayload")) {
				...
				auto dropItem = *static_cast<PipelineItem**>(payload->Data);
				bool duplicate = ImGui::GetIO().KeyCtrl;

				// first find a name that is not used
				auto name = std::string(dropItem->Name);
				...
				if (dropItem->Type == PipelineItem::ItemType::Geometry) {
					auto* newData = new pipe::GeometryItem();
					auto* origData = static_cast<pipe::GeometryItem*>(dropItem->Data);
				...
				// duplicate Model:
					...
					auto* newData = new pipe::Model();
					auto origData = static_cast<pipe::Model*>(dropItem->Data);
				...
				// duplicate VertexBuffer:
					...
					auto* origData = static_cast<pipe::VertexBuffer*>(dropItem->Data);
					...
					if (newData->Buffer != 0)
						gl::CreateBufferVAO(newData->VAO, static_cast<ed::BufferObject*>(newData->Buffer)->ID, m_data->Objects.ParseBufferFormat(((ed::BufferObject*)newData->Buffer)->ViewFormat));
				...
				// duplicate RenderState:
					...
					auto* newData = new pipe::RenderState();
					auto* origData = static_cast<pipe::RenderState*>(dropItem->Data);
				...
				// duplicate PluginItem:
					...
					auto* newData = new pipe::PluginItemData();
					auto* origData = static_cast<pipe::PluginItemData*>(dropItem->Data);
				...
				if (!duplicate) {
					...
					auto prev = dynamic_cast<PreviewUI*>(m_ui->Get(ViewID::Preview));
			...
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("ObjectPayload")) {
				ed::ObjectManagerItem* dropItem = *(static_cast<ed::ObjectManagerItem**>(payload->Data));
	...
	void PipelineUI::m_addComputePass(ed::PipelineItem* item) const
	{
		auto* data = static_cast<ed::pipe::ComputePass*>(item->Data);
		...
		if (int ewCount = m_data->Messages.GetGroupErrorAndWarningMsgCount(item->Name); ewCount > 0)
		...
		if (ImGui::BeginDragDropTarget()) {
			// object
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("ObjectPayload")) {
				ed::ObjectManagerItem* dropItem = *(static_cast<ObjectManagerItem**>(payload->Data));
				m_handleObjectDrop(item, dropItem);
			}
			ImGui::EndDragDropTarget();
		}
	...
	void PipelineUI::m_addAudioPass(ed::PipelineItem* item) const
	{
		auto* data = static_cast<ed::pipe::AudioPass*>(item->Data);
	...
	void PipelineUI::m_addPluginItem(ed::PipelineItem* item) const
	{
		auto* data = static_cast<ed::pipe::PluginItemData*>(item->Data);
		...
		if (ImGui::BeginDragDropTarget()) {
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("PipelineItemPayload")) {
				// TODO: m_data->Pipeline.DuplicateItem() ?
				PipelineItem* dropItem = *(static_cast<ed::PipelineItem**>(payload->Data));

				if (data->Owner->PipelineItem_CanHaveChild(data->Type, data->PluginData, static_cast<plugin::PipelineItemType>(dropItem->Type))) {

					const bool duplicate = ImGui::GetIO().KeyCtrl;
					...
					if (duplicate) { // change
						...
						// add a number to the string and check if it already exists
						for (size_t j = 2; /*WE WILL BREAK FROM INSIDE ONCE WE FIND THE NAME*/; j++) {
							std::string newName = name + std::to_string(j);

							if (const bool has = m_data->Pipeline.Has(newName.c_str()); !has) {
								name = newName;
								break;
							}
						}
					...
					if (dropItem->Type == PipelineItem::ItemType::Geometry) {
						auto* newData = new pipe::GeometryItem();
						auto* origData = (pipe::GeometryItem*)dropItem->Data;
					...
					// duplicate Model:
						...
						auto* newData = new pipe::Model();
						auto* origData = (pipe::Model*)dropItem->Data;
					...
					// duplicate RenderState:
						...
						auto* newData = new pipe::RenderState();
						const auto origData = static_cast<pipe::RenderState*>(dropItem->Data);
						memcpy(newData, origData, sizeof(pipe::RenderState));
	...
	void PipelineUI::m_addItem(ed::PipelineItem* item) const
	{
		auto* pluginData = (pipe::PluginItemData*)item->Data;
		const bool isPluginItem = item->Type == PipelineItem::ItemType::PluginItem;
		const bool hasPluginProperties = isPluginItem && pluginData->Owner->PipelineItem_HasProperties(pluginData->Type, pluginData->PluginData);
		const bool isPluginPickable = isPluginItem && pluginData->Owner->PipelineItem_IsPickable(pluginData->Type, pluginData->PluginData);
			...
			if (ImGui::IsMouseDoubleClicked(0)) {
				...
				if (Settings::Instance().General.SelectItemOnDblClk) {
					if (item->Type == PipelineItem::ItemType::Geometry || item->Type == PipelineItem::ItemType::Model || item->Type == PipelineItem::ItemType::VertexBuffer || isPluginPickable) {
						...
						if (proceed) {
							auto props = reinterpret_cast<PreviewUI*>(m_ui->Get(ViewID::Preview));
							props->Pick(item);
	...
	void PipelineUI::m_handleObjectDrop(ed::PipelineItem* pass, ed::ObjectManagerItem* object) const
	{
		Logger::Get().Log("Handling an object drop on pipeline item");

		const bool isPluginOwner = object->Plugin != nullptr;
		const PluginObject* pobj = object->Plugin;
