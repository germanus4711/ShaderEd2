...
namespace ed {
			...
			for (auto& suggestion : suggestions) {
				...
				if (ImGui::BeginChild("##suggestion_container", ImVec2(0, m_suggestionHeights[sugId]), true)) {
					...
					if (suggestion.Type == DebuggerSuggestion::SuggestionType::ComputeShader) {
						const auto pass = static_cast<pipe::ComputePass*>(suggestion.Item->Data);
						const glm::ivec3 thread = suggestion.Thread;
						const ed::ShaderLanguage lang = ed::ShaderCompiler::GetShaderLanguageFromExtension(pass->Path);
						...
						if (ImGui::Button(UI_ICON_PLAY "##debug_compute", ImVec2(ICON_BUTTON_WIDTH, BUTTON_SIZE))) {
							...
							auto pass = static_cast<pipe::ComputePass*>(suggestion.Item->Data);
							// TODO: plugins?
						...
						/* THREAD INFO */
						const int localInvocationIndex = (thread.z % suggestion.WorkgroupSize.z) * suggestion.WorkgroupSize.x * suggestion.WorkgroupSize.z + (thread.y % suggestion.WorkgroupSize.y) * suggestion.WorkgroupSize.x + (thread.x % suggestion.WorkgroupSize.x);
	...
	void PixelInspectUI::StartDebugging(TextEditor* editor, const PluginShaderEditor& pluginEditor, PixelInformation* pixel)
	{
		...
		if (editor != nullptr)
			...
			dynamic_cast<IPlugin3*>(pluginEditor.Plugin)->ShaderEditor_SetLineIndicator(pluginEditor.LanguageID, pluginEditor.ID, m_data->Debugger.GetCurrentLine());

		if (pixel) {
			if (editor != nullptr) {
				m_data->Plugins.HandleApplicationEvent(ed::plugin::ApplicationEvent::DebuggerStarted, pixel->Pass->Name, editor);
			} else {
				m_data->Plugins.HandleApplicationEvent(ed::plugin::ApplicationEvent::DebuggerStarted, pixel->Pass->Name, (void*)&pluginEditor);
			}
		}

		// reset function stack
		dynamic_cast<DebugFunctionStackUI*>(m_ui->Get(ViewID::DebugFunctionStack))->Refresh();

		// update variable values
		dynamic_cast<DebugValuesUI*>(m_ui->Get(ViewID::DebugValues))->Refresh();
		...
		if (editor) {
			editor->OnDebuggerAction = [&](TextEditor* ed, const TextEditor::DebugAction act) {
				...
				case TextEditor::DebugAction::StepOut:
					...
					break;
				case TextEditor::DebugAction::Stop: break;
				}
				...
				if (act == TextEditor::DebugAction::Stop || !state) {
					...
					dynamic_cast<DebugWatchUI*>(m_ui->Get(ViewID::DebugWatch))->Refresh();
					dynamic_cast<DebugVectorWatchUI*>(m_ui->Get(ViewID::DebugVectorWatch))->Refresh();
					dynamic_cast<DebugFunctionStackUI*>(m_ui->Get(ViewID::DebugFunctionStack))->Refresh();
					dynamic_cast<DebugValuesUI*>(m_ui->Get(ViewID::DebugValues))->Refresh();
					...
					m_data->Plugins.HandleApplicationEvent(ed::plugin::ApplicationEvent::DebuggerStepped, reinterpret_cast<void*>(static_cast<intptr_t>(curLine)), nullptr);
				}
			};
			...
			editor->OnIdentifierHover = [&](TextEditor* ed, const std::string& id) {
				...
				if (res != nullptr && resType != nullptr) {
					...
					if (isTex && res->value.image != nullptr) {
						spvm_image_t tex = res->value.image;

						if (isCube) {
							m_cubePrev.Draw(static_cast<GLuint>(reinterpret_cast<uintptr_t>(tex->data)));
							ImGui::Image(reinterpret_cast<ImTextureID>(static_cast<uintptr_t>(m_cubePrev.GetTexture())), ImVec2(128.0f, 128.0f * (375.0f / 512.0f)), ImVec2(0, 1), ImVec2(1, 0));
						} else if (isTex3D) {
							const float imgWH = (static_cast<float>(tex->height) / static_cast<float>(tex->width));
							m_tex3DPrev.Draw(static_cast<GLuint>(reinterpret_cast<uintptr_t>(tex->data)), 128.0f, static_cast<int>(128.0f * imgWH));
							ImGui::Image(reinterpret_cast<void*>(static_cast<intptr_t>(m_tex3DPrev.GetTexture())), ImVec2(128.0f, 128.0f * imgWH));
						} else
							ImGui::Image((ImTextureID)tex->data, ImVec2(128.0f, 128.0f * (static_cast<float>(tex->height) / static_cast<float>(tex->width))), ImVec2(0, 1), ImVec2(1, 0));
					} else {
						// color preview
						...
						// text value
						std::stringstream ss;
						m_data->Debugger.GetVariableValueAsString(ss, m_data->Debugger.GetVM(), resType, res, static_cast<int>(vcount), "");
			...
			editor->HasExpressionHover = [&](TextEditor* ed, const std::string& id) -> bool {
				...
				if (m_cacheExists) {
					std::stringstream ss;
					m_data->Debugger.GetVariableValueAsString(ss, m_data->Debugger.GetVMImmediate(), resType, exprVal == nullptr ? nullptr : exprVal->members, exprVal == nullptr ? 0 : exprVal->member_count, "");
					...
					if (resType->value_type == spvm_value_type_vector && m_data->Debugger.GetVMImmediate()->results[resType->pointer].value_type == spvm_value_type_float) {
						if ((resType->member_count == 3) || (resType->member_count == 4))
							m_cacheHasColor = true;
						for (int i = 0; i < resType->member_count; i++)
							if (!(exprVal == nullptr || exprVal->members[i].value.f != 0.0f))
			...
			editor->OnExpressionHover = [&](TextEditor* ed, const std::string& id) {
				...
				ImGui::Text("%s", m_cacheValue.c_str());
			};
		}

		// copy preview camera info to vertex watch camera & geometry shader output camera
		if (!Settings::Instance().Project.FPCamera) {
			auto previewCamera = dynamic_cast<ArcBallCamera*>(SystemVariableManager::Instance().GetCamera());

			auto vectorWatchUI = dynamic_cast<DebugVectorWatchUI*>(m_ui->Get(ViewID::DebugVectorWatch));
			ArcBallCamera* vectorCamera = vectorWatchUI->GetCamera();

			auto geometryOutputUI = dynamic_cast<DebugGeometryOutputUI*>(m_ui->Get(ViewID::DebugGeometryOutput));
