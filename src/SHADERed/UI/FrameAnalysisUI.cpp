...
namespace ed {
	void FrameAnalysisUI::Process()
	{
		...
		auto* pixels = static_cast<uint8_t*>(malloc(rendererSize.x * rendererSize.y * 4));
		...
		for (int x = 0; x < rendererSize.x; x++) {
			for (int y = 0; y < rendererSize.y; y++) {
				const uint8_t* px = &pixels[(y * rendererSize.x + x) * 4];
	...
	void FrameAnalysisUI::Update(float delta)
	{
		...
		ImGui::PlotHistogram("##testhistogram", histogramData, 256, 0, nullptr, 0.0f, FLT_MAX, ImVec2(ImGui::GetContentRegionAvailWidth(), 256));
		ImGui::NewLine();
		...
		if (pixelHistoryCount > 0) {
			...
			for (auto& pixel : pixels) {
				...
				dynamic_cast<PixelInspectUI*>(m_ui->Get(ViewID::PixelInspect))->RenderPixelInfo(pixel, m_pixelHeights[pxId]);
