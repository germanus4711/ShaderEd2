	...
	void BrowseOnlineUI::Update(float delta)
	{
		...
		if (m_refreshShaderThumbnails) {
			if (m_queueThumbnailProcess) {
				...
				if (m_onlineShaderThumbnail.size() < std::min<int>(12, (int)m_onlineShaders.size()))
		...
		int startY = (int)ImGui::GetCursorPosY();
		ImGui::Text("Query:");
		ImGui::SameLine();
		...
		ImGui::PopItemWidth();
		// int startX = (int)ImGui::GetCursorPosX();
		...
		if (ImGui::BeginTabBar("BrowseOnlineTabBar")) {
			if (ImGui::BeginTabItem("Shaders")) {
				...
				if (ImGui::BeginTable("##shaders_table", 2, ImGuiTableFlags_None)) {
					...
					for (int i = 0; i < std::min<int>(12, (int)m_onlineShaders.size()); i++) {
						...
						if (i < m_onlineShaderThumbnail.size())
							ImGui::Image((ImTextureID)(intptr_t)m_onlineShaderThumbnail[i], ImVec2(256, 144), ImVec2(0, 1), ImVec2(1, 0));
						else {
			...
			if (ImGui::BeginTabItem("Plugins")) {
				...
				if (m_refreshPluginThumbnails) {
					int minSize = std::min<int>(12, (int)m_onlinePlugins.size());
					...
					for (int i = 0; i < minSize; i++) {
						...
						unsigned char* pixelData = stbi_load_from_memory(reinterpret_cast<stbi_uc*>(data), (int)dataLen, &width, &height, &nrChannels, STBI_rgb_alpha);
				...
				if (ImGui::BeginTable("##plugins_table", 2, ImGuiTableFlags_None)) {
					...
					for (int i = 0; i < std::min<int>(12, (int)m_onlinePlugins.size()); i++) {
						const ed::WebAPI::PluginResult& pluginInfo = m_onlinePlugins[i];

						ImGui::TableNextRow();

						ImGui::TableSetColumnIndex(0);
						ImGui::Image((ImTextureID)(intptr_t)m_onlinePluginThumbnail[i], ImVec2(256, 144), ImVec2(0, 1), ImVec2(1, 0));
			...
			if (ImGui::BeginTabItem("Themes")) {
				...
				if (m_refreshThemeThumbnails) {
					int minSize = std::min<int>(12, (int)m_onlineThemes.size());
				...
				if (ImGui::BeginTable("##themes_table", 2, ImGuiTableFlags_None)) {
					...
					for (int i = 0; i < std::min<int>(12, (int)m_onlineThemes.size()); i++) {
