...
namespace ed {
	BrowseOnlineUI::BrowseOnlineUI(GUIManager* ui, ed::InterfaceManager* objects, const std::string& name, const bool visible)
	...
	// void BrowseOnlineUI::LoadThumbnails(bool m_queueThumbnailProcess,bool m_queueThumbnailNext, bool m_refreshShaderThumbnails, void* m_queueThumbnailPixelData, int m_queueThumbnailWidth, int m_queueThumbnailHeight)
	void BrowseOnlineUI::LoadThumbnails()
	{
		// load thumbnails 1 by 1 in a separate thread
		if (m_refreshShaderThumbnails) {
			if (m_queueThumbnailProcess) {
				...
				if (m_onlineShaderThumbnail.size() < std::min<int>(12, static_cast<int>(m_onlineShaders.size())))
			...
			if (m_queueThumbnailNext) {
				m_data->API.AllocateShaderThumbnail(m_onlineShaders[m_queueThumbnailID].ID, [&](unsigned char* data, const int width, const int height) {
	...
	void BrowseOnlineUI::Update(float delta)
	{
		...
		int startY = static_cast<int>(ImGui::GetCursorPosY());
		ImGui::Text("Query:");
		ImGui::SameLine();
		...
		ImGui::PopItemWidth();
		int startX = static_cast<int>(ImGui::GetCursorPosX());
		...
		if (!ImGui::BeginTabBar("BrowseOnlineTabBar")) return;
		...
		if (ImGui::BeginTabItem("Shaders")) {
			...
			if (ImGui::BeginTable("##shaders_table", 2, ImGuiTableFlags_None)) {
				...
				for (int i = 0; i < std::min<int>(12, static_cast<int>(m_onlineShaders.size())); i++) {
					...
					if (i < m_onlineShaderThumbnail.size())
						ImGui::Image(reinterpret_cast<ImTextureID>(static_cast<intptr_t>(m_onlineShaderThumbnail[i])), ImVec2(256, 144), ImVec2(0, 1), ImVec2(1, 0));
					else {
						ImVec2 spinnerPos = ImGui::GetCursorPos();
						ImGui::Dummy(ImVec2(256, 144));
						ImGui::SetCursorPos(ImVec2(spinnerPos.x + static_cast<float>(256) / 2 - 32, spinnerPos.y + static_cast<float>(144) / 2 - 32));
						UIHelper::Spinner("##spinner", 32.0f, 6, ImGui::GetColorU32(ImGuiCol_Text));
					}

					ImGui::TableSetColumnIndex(1);
					ImGui::Text("%s", shaderInfo.Title.c_str()); // just in case someone has an %s or sth in the title, so that the app doesn't crash: 'D
					...
					if (ImGui::Button("OPEN")) {
						...
						if (m_data->API.DownloadShaderProject(shaderInfo.ID)) {
		...
		if (ImGui::BeginTabItem("Plugins")) {
			...
			if (m_refreshPluginThumbnails) {
				int minSize = std::min<int>(12, static_cast<int>(m_onlinePlugins.size()));
				...
				for (int i = 0; i < minSize; i++) {
					...
					unsigned char* pixelData = stbi_load_from_memory(reinterpret_cast<stbi_uc*>(data), static_cast<int>(dataLen), &width, &height, &nrChannels, STBI_rgb_alpha);
			...
			if (ImGui::BeginTable("##plugins_table", 2, ImGuiTableFlags_None)) {
				...
				for (int i = 0; i < std::min<int>(12, static_cast<int>(m_onlinePlugins.size())); i++) {
					const WebAPI::PluginResult& pluginInfo = m_onlinePlugins[i];

					ImGui::TableNextRow();

					ImGui::TableSetColumnIndex(0);
					ImGui::Image(reinterpret_cast<ImTextureID>(static_cast<intptr_t>(m_onlinePluginThumbnail[i])), ImVec2(256, 144), ImVec2(0, 1), ImVec2(1, 0));

					ImGui::TableSetColumnIndex(1);
					ImGui::Text("%s", pluginInfo.Title.c_str()); // just in case someone has an %s or sth in the title, so that the app doesn't crash: 'D
		...
		if (ImGui::BeginTabItem("Themes")) {
			...
			if (m_refreshThemeThumbnails) {
				int minSize = std::min<int>(12, static_cast<int>(m_onlineThemes.size()));
			...
			if (ImGui::BeginTable("##themes_table", 2, ImGuiTableFlags_None)) {
				...
				for (int i = 0; i < std::min<int>(12, static_cast<int>(m_onlineThemes.size())); i++) {
					const WebAPI::ThemeResult& themeInfo = m_onlineThemes[i];

					ImGui::TableNextRow();

					ImGui::TableSetColumnIndex(0);
					ImGui::Image(reinterpret_cast<ImTextureID>(static_cast<intptr_t>(m_onlineThemeThumbnail[i])), ImVec2(256, 144), ImVec2(0, 1), ImVec2(1, 0));
					ImGui::TableSetColumnIndex(1);
					...
					const auto& tList = dynamic_cast<OptionsUI*>(m_ui->Get(ViewID::Options))->GetThemeList();

					if (std::count(tList.begin(), tList.end(), themeInfo.Title) == 0) {
						ImGui::PushID(i);
						if (ImGui::Button("DOWNLOAD")) {
							m_data->API.DownloadTheme(themeInfo.ID);
							dynamic_cast<OptionsUI*>(m_ui->Get(ViewID::Options))->RefreshThemeList();
						}
						ImGui::PopID();
					} else {
						ImGui::PushID(i);
						if (ImGui::Button("APPLY")) {
							...
							dynamic_cast<OptionsUI*>(m_ui->Get(ViewID::Options))->ApplyTheme();
						}
						ImGui::PopID();
					}
				}
	...
	void BrowseOnlineUI::Open()
	{
		...
		if (m_onlineShaders.empty())
			m_onlineSearchShaders();
		if (m_onlinePlugins.empty())
			m_onlineSearchPlugins();
		if (m_onlineThemes.empty())
			m_onlineSearchThemes();
	}
	void BrowseOnlineUI::FreeMemory()
	{
		for (unsigned int& i : m_onlineShaderThumbnail)
			glDeleteTextures(1, &i);
		m_onlineShaderThumbnail.clear();

		for (unsigned int& i : m_onlinePluginThumbnail)
			glDeleteTextures(1, &i);
	...
	void BrowseOnlineUI::m_onlineSearchThemes()
	{
		...
		for (unsigned int& i : m_onlineThemeThumbnail)
			glDeleteTextures(1, &i);
