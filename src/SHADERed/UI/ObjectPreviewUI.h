...
namespace ed {
	...
	public:
		...
		~ObjectPreviewUI() override = default;
		...
		[[nodiscard]] bool ShouldRun() const { return !m_items.empty(); }
		void CloseAll();
		void Close(const std::string& name);

	private:
		eng::Timer m_bufUpdateClock;
		static bool m_drawBufferElement(int row, int col, void* data, ShaderVariable::ValueType type);
