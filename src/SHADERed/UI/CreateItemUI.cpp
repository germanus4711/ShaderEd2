...
namespace ed {
	...
	void CreateItemUI::Update(float delta)
	{
		...
		if (m_item.Type == PipelineItem::ItemType::Geometry) {
			auto* data = static_cast<pipe::GeometryItem*>(m_item.Data);

			// geo type
			ImGui::Text("Geometry type:");
			ImGui::NextColumn();
			ImGui::PushItemWidth(-1);
			ImGui::Combo("##cui_geotype", reinterpret_cast<int*>(&data->Type), GEOMETRY_NAMES, std::size(GEOMETRY_NAMES));
			ImGui::NextColumn();
			...
			auto* data = static_cast<pipe::VertexBuffer*>(m_item.Data);
			...
			auto* data = static_cast<pipe::ComputePass*>(m_item.Data);
			...
			auto* data = static_cast<pipe::AudioPass*>(m_item.Data);
			...
			auto* data = static_cast<pipe::RenderState*>(m_item.Data);
			...
			auto* data = static_cast<pipe::Model*>(m_item.Data);
	...
	void CreateItemUI::SetType(PipelineItem::ItemType type)
	{
		...
		if (m_item.Type == PipelineItem::ItemType::Geometry) {
			...
			auto* allocatedData = new pipe::VertexBuffer();
			allocatedData->Scale = glm::vec3(1, 1, 1);
			allocatedData->Buffer = nullptr;
			m_item.Data = allocatedData;
		} else if (m_item.Type == PipelineItem::ItemType::ShaderPass) {
			Logger::Get().Log("Opening a CreateItemUI for creating ShaderPass object...");

			auto* allocatedData = new pipe::ShaderPass();
			...
			auto* allocatedData = new pipe::ComputePass();
			...
			auto* allocatedData = new pipe::AudioPass();
			m_item.Data = allocatedData;
			m_item.Name[0] = 0;
		} else if (m_item.Type == PipelineItem::ItemType::RenderState) {
			Logger::Get().Log("Opening a CreateItemUI for creating RenderState object...");

			auto* allocatedData = new pipe::RenderState();
			m_item.Data = allocatedData;
		} else if (m_item.Type == PipelineItem::ItemType::Model) {
			...
			auto* allocatedData = new pipe::Model();
	...
	bool CreateItemUI::Create()
	{
		...
		if (m_item.Type == PipelineItem::ItemType::ShaderPass) {
			auto* data = new pipe::ShaderPass();
			const auto* origData = static_cast<pipe::ShaderPass*>(m_item.Data);
			...
			if (!m_errorOccured) {
				...
				auto* code = dynamic_cast<CodeEditorUI*>(m_ui->Get(ViewID::Code));
			...
			auto* data = new pipe::ComputePass();
			const auto* origData = static_cast<pipe::ComputePass*>(m_item.Data);
			...
			auto* data = new pipe::AudioPass();
			auto* origData = static_cast<pipe::AudioPass*>(m_item.Data);
			...
			if (m_item.Type == PipelineItem::ItemType::Geometry) {
				...
				auto* origData = static_cast<pipe::GeometryItem*>(m_item.Data);
				...
				if (ownerItem->Type == PipelineItem::ItemType::ShaderPass)
					inpLayout = static_cast<pipe::ShaderPass*>(ownerItem->Data)->InputLayout;
				else if (ownerItem->Type == PipelineItem::ItemType::PluginItem) {
					const auto pdata = static_cast<pipe::PluginItemData*>(ownerItem->Data);
				...
				auto* data = new pipe::RenderState();
				const auto* origData = static_cast<pipe::RenderState*>(m_item.Data);
				...
				const auto* origData = static_cast<pipe::Model*>(m_item.Data);
				...
				auto* data = new pipe::VertexBuffer();
				const auto* origData = static_cast<pipe::VertexBuffer*>(m_item.Data);
				...
				if (data->Buffer != nullptr)
					gl::CreateBufferVAO(data->VAO, static_cast<ed::BufferObject*>(data->Buffer)->ID, m_data->Objects.ParseBufferFormat(static_cast<ed::BufferObject*>(data->Buffer)->ViewFormat));
	...
	void CreateItemUI::UpdateLanguageList()
	{
		...
		if (!Settings::Instance().General.HLSLExtensions.empty())
			m_fileAutoExtensions[1] = Settings::Instance().General.HLSLExtensions[0];
		if (!Settings::Instance().General.VulkanGLSLExtensions.empty())
		...
		for (IPlugin1* pl : plugins) {
			const int langCount = pl->CustomLanguage_GetCount();
			for (int i = 0; i < langCount; i++) {
				...
				if (!extVec.empty())
					...
					m_fileAutoExtensions.emplace_back("null");
			}
		}
	}
	void CreateItemUI::m_createFile(const std::string& filename) const
	{
