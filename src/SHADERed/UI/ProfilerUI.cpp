...
namespace ed {
	...
	{
		const float barWidth = static_cast<float>(time) / static_cast<float>(totalTime);
		const float barX = static_cast<float>(timeOffset) / static_cast<float>(totalTime);

		const float windowWidth = ImGui::GetWindowWidth();
		const float barMaxWidth = (3 * windowWidth) / 4 - 2 * PROFILER_PADDING;

		const float rowHeight = static_cast<float>(Settings::Instance().General.FontSize) + 2 * PROFILER_PADDING;
		const float rowPosY = rowHeight * static_cast<float>(index);
		...
		// time text
		char timeText[16] = { 0 };
		snprintf(timeText, sizeof(timeText), "%.4f ms", static_cast<float>(time) / 1000000.0f);
