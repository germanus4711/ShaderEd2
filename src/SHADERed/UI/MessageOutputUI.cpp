...
namespace ed {
	...
	void MessageOutputUI::Update(float delta)
	{
		...
		if (ImGui::BeginTable("##msg_table", 4, ImGuiTableFlags_Resizable | ImGuiTableFlags_ScrollFreezeTopRow | ImGuiTableFlags_ScrollY)) {
			...
			// int rowIndex = 0;
			const CustomColors& clrs = ThemeContainer::Instance().GetCustomStyle(Settings::Instance().Theme);
			...
			for (int i = 0; i < msgs.size(); i++) {
				...
				if (ImGui::Selectable(m->Group.c_str(), false, ImGuiSelectableFlags_SpanAllColumns | ImGuiSelectableFlags_AllowDoubleClick)) {
					if (ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left)) {
						PipelineItem* pass = m_data->Pipeline.Get(m->Group.c_str());
				...
				if (m->Shader != ShaderStage::Count) {
					...
					if (m->Shader == ShaderStage::Pixel)
						stageAbbr = "PS";
					else if (m->Shader == ShaderStage::Geometry)
						stageAbbr = "GS";
					else if (m->Shader == ShaderStage::Compute)
						stageAbbr = "CS";
					else if (m->Shader == ShaderStage::Audio)
						stageAbbr = "AS";
					else if (m->Shader == ShaderStage::TessellationControl)
						stageAbbr = "TCS";
					else if (m->Shader == ShaderStage::TessellationEvaluation)
						stageAbbr = "TES";
				...
				if (m->Line != -1)
					ImGui::Text("%s", std::to_string(m->Line).c_str());

				ImGui::TableSetColumnIndex(3);
				ImGui::TextColored(color, "%s", m->Text.c_str());
			}
