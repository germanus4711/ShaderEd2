...
namespace ed {
	...
	{
		auto containerSize = ImVec2(ImGui::GetWindowContentRegionWidth(), abs(ImGui::GetWindowContentRegionMax().y - ImGui::GetWindowContentRegionMin().y));
		...
		for (auto& item : items) {
			...
			if (oItem->Type == ObjectType::RenderTexture) {
				...
				imgWH = static_cast<float>(rtSize.y) / static_cast<float>(rtSize.x);
			} else if (oItem->Type == ObjectType::Audio)
				imgWH = 2.0f / 512.0f;
			else if (oItem->Type == ObjectType::CubeMap)
				imgWH = 375.0f / 512.0f;
			else if (oItem->Type == ObjectType::Image)
				imgWH = static_cast<float>(oItem->Image->Size.y) / static_cast<float>(oItem->Image->Size.x);
			else if (oItem->Type == ObjectType::Image3D)
				imgWH = static_cast<float>(oItem->Image3D->Size.y) / static_cast<float>(oItem->Image3D->Size.x);
			else if (oItem->Type != ObjectType::PluginObject)
				imgWH = static_cast<float>(oItem->TextureSize.y) / static_cast<float>(oItem->TextureSize.x);
			...
			if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
				...
				if (oItem->Type == ObjectType::CubeMap) {
					m_cubePrev.Draw(tex);
					ImGui::Image(reinterpret_cast<void*>(static_cast<intptr_t>(m_cubePrev.GetTexture())), ImVec2(IMAGE_CONTEXT_WIDTH, ((float)imgWH) * IMAGE_CONTEXT_WIDTH), ImVec2(0, 1), ImVec2(1, 0));
				} else if (!isBuf && !isImg3D && !isPluginOwner)
					ImGui::Image(reinterpret_cast<void*>(static_cast<intptr_t>(tex)), ImVec2(IMAGE_CONTEXT_WIDTH, ((float)imgWH) * IMAGE_CONTEXT_WIDTH), ImVec2(0, 1), ImVec2(1, 0));
			...
			if (ImGui::BeginPopupContextItem(std::string("##context" + item->Name).c_str())) {
				itemMenuOpened = true;
				...
				if (oItem->Image != nullptr || isImg3D) {
					if (ImGui::BeginMenu(isImg3D ? "Bind UAV/image3D" : "Bind UAV/image2D")) {
						for (auto passe : passes) {
							if (passe->Type == PipelineItem::ItemType::ComputePass) {
								int boundID = m_data->Objects.IsUniformBound(oItem, passe);
								size_t boundItemCount = m_data->Objects.GetUniformBindList(passe).size();
								bool isBound = boundID != -1;
								if (ImGui::MenuItem(passe->Name, ("(" + std::to_string(boundID == -1 ? boundItemCount : boundID) + ")").c_str(), isBound)) {
									if (!isBound)
										m_data->Objects.BindUniform(oItem, passe);
									else
										m_data->Objects.UnbindUniform(oItem, passe);
								}
							}
						}
						ImGui::EndMenu();
				...
				if ((isPluginObjBindable || !isPluginOwner) && ImGui::BeginMenu("Bind")) {
					...
					if (isBuf || isPluginObjUAV) {
						for (auto passe : passes) {
							int boundID = m_data->Objects.IsUniformBound(oItem, passe);
							size_t boundItemCount = m_data->Objects.GetUniformBindList(passe).size();
							if (bool isBound = boundID != -1; ImGui::MenuItem(passe->Name, ("(" + std::to_string(boundID == -1 ? boundItemCount : boundID) + ")").c_str(), isBound)) {
								if (!isBound)
									m_data->Objects.BindUniform(oItem, passe);
								else
									m_data->Objects.UnbindUniform(oItem, passe);
						...
						for (auto passe : passes) {
							int boundID = m_data->Objects.IsBound(oItem, passe);
							size_t boundItemCount = m_data->Objects.GetBindList(passe).size();
							if (bool isBound = boundID != -1; ImGui::MenuItem(passe->Name, ("(" + std::to_string(boundID == -1 ? boundItemCount : boundID) + ")").c_str(), isBound)) {
								if (!isBound)
									m_data->Objects.Bind(oItem, passe);
								else
									m_data->Objects.Unbind(oItem, passe);
							}
						}
					}
					ImGui::EndMenu();
				...
				if (oItem->Type == ObjectType::RenderTexture || oItem->Type == ObjectType::Image || oItem->Type == ObjectType::Texture || oItem->Type == ObjectType::Texture3D || isImg3D || (isPluginOwner && pobj->Owner->Object_HasProperties(pobj->Type))) {
					if (ImGui::Selectable("Properties"))
						dynamic_cast<PropertyUI*>(m_ui->Get(ViewID::Properties))->Open(oItem);
				}
				...
				if (ImGui::Selectable("Delete")) {
					if (oItem->Type == ObjectType::RenderTexture) {
						auto& _passes = m_data->Pipeline.GetList();
						for (auto& _passe : _passes) {
							if (_passe->Type != PipelineItem::ItemType::ShaderPass)
								continue;

							auto* sData = static_cast<pipe::ShaderPass*>(_passe->Data);

							// check if this shader pass has a window bound to it
							// TODO this looks unused vvvv
							bool alreadyUsesWindow = false;
							for (unsigned int RenderTexture : sData->RenderTextures)
								if (RenderTexture == m_data->Renderer.GetTexture()) {
					...
					if (isBuf) {
						...
						for (auto& _passe : _passes) {
							...
							const auto* pdata = static_cast<pipe::ShaderPass*>(_passe->Data);
							for (int k = 0; k < pdata->Items.size(); k++) {
								const PipelineItem* pitem = pdata->Items[k];
								if (pitem->Type == ed::PipelineItem::ItemType::Geometry) {
									auto* gitem = static_cast<pipe::GeometryItem*>(pitem->Data);

									if (gitem->InstanceBuffer == static_cast<void*>(oItem->Buffer))
									...
									if (mitem->InstanceBuffer == static_cast<void*>(oItem->Buffer)) {
									...
									auto* vbitem = static_cast<pipe::VertexBuffer*>(pitem->Data);
					...
					dynamic_cast<ObjectPreviewUI*>(m_ui->Get(ViewID::ObjectPreview))->Close(oItem->Name);

					const auto props = dynamic_cast<PropertyUI*>(m_ui->Get(ViewID::Properties));
