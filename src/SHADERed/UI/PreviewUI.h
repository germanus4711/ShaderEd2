...
namespace ed {
	class PreviewUI final : public UIView {
	public:
		...
		[[nodiscard]] glm::vec2 GetUIRectSize() const { return { m_imgSize.x, m_imgSize.y }; }
		[[nodiscard]] glm::vec2 GetUIRectPosition() const { return { m_imgPosition.x, m_imgPosition.y }; }

		void SetVariableValue(PipelineItem* item, const std::string& varName, int line);
		[[nodiscard]] bool IsFrameAnalyzed() const { return m_frameAnalyzed; }
