...
namespace ed {
	...
	void OptionsUI::Update(float delta)
	{
		...
		if (ifd::FileDialog::Instance().IsDone("AddIncludeDirDlg")) {
			if (ifd::FileDialog::Instance().HasResult()) {
				...
				bool exists = false;
				for (const auto & IncludePath : Settings::Instance().Project.IncludePaths)
					if (IncludePath == ipath) {
						exists = true;
						break;
					}
				if (!exists) {
	...
	void OptionsUI::ApplyTheme() const
	{
	...
	std::string OptionsUI::m_getShortcutString() const
	{
		std::string ret;
	...
	void OptionsUI::m_loadPluginList()
	{
		...
		for (const auto & i : m_pluginsNotLoaded) {
			for (int j = 0; j < m_pluginsLoaded.size(); j++) {
				if (m_pluginsLoaded[j] == i) {
	...
	void OptionsUI::m_loadThemeList()
	{
		...
		m_themes.clear();
		m_themes.emplace_back("Dark");
		m_themes.emplace_back("Light");

		std::vector<std::string> pathList = {
			"./themes/"
		};
		if (!ed::Settings::Instance().LinuxHomeDirectory.empty())
			pathList.push_back(ed::Settings::Instance().LinuxHomeDirectory + "themes/");
	...
	void OptionsUI::m_renderPlugins()
	{
		...
		ImGui::PushItemWidth(-1);
		ImGui::ListBox("##optpl_loaded", &m_pluginLoadedLB, vector_getter, (void*)(&m_pluginsLoaded), (int)m_pluginsLoaded.size(), 10);
		ImGui::PopItemWidth();
		ImGui::NextColumn();
	...
	void OptionsUI::m_renderCodeSnippets()
	{
		...
		if (ImGui::BeginTable("##optcs_table", 4, ImGuiTableFlags_BordersInner | ImGuiTableFlags_Resizable)) {
			...
			for (const auto& snippet : snippets) {
				...
				ImGui::Text("%s", snippet.Display.c_str());
				ImGui::TableSetColumnIndex(2);
				ImGui::Text("%s", snippet.Search.c_str());
				ImGui::TableSetColumnIndex(3);
				ImGui::Text("%s", snippet.Code.c_str());
				rowIndex++;
			}
			ImGui::EndTable();
		}
