...
namespace ed {
	void DebugDrawPrimitives(int& vertexStart, int vertexCount, int maxVertexCount, int vertexStrip, GLuint topology, GLuint varLoc, bool instanced, int instanceCount, bool useIndices = false, int vbase = 0)
	{
		...
		while (vertexStart < maxVertexCount) {
			float r = (float)((vertexStart + vbase) & 0x000000FF) / 255.0f;
			float g = (float)(((vertexStart + vbase) & 0x0000FF00) >> 8) / 255.0f;
			float b = (float)(((vertexStart + vbase) & 0x00FF0000) >> 16) / 255.0f;
			glUniform4f((GLint)varLoc, r, g, b, 0.0f);
	...
	{
		while (iStart < iCount) {
			float r = (float)(iStart & 0x000000FF) / 255.0f;
			float g = (float)((iStart & 0x0000FF00) >> 8) / 255.0f;
			float b = (float)((iStart & 0x00FF0000) >> 16) / 255.0f;
			glUniform4f((GLint)varLoc, r, g, b, 0.0f);
			...
			, m_wasMultiPick(false)
	{
		...
		if (bool isDebugShaderCompiled = gl::CheckShaderCompilationStatus(m_generalDebugShader, msg); !isDebugShaderCompiled)
	...
	void RenderEngine::Render(int width, int height, bool isDebug, PipelineItem* breakItem)
	{
		...
		GLuint previousTexture[MAX_RENDER_TEXTURES] = { 0 }; // don't clear the render target if we use it two times in a row
		...
		if (performPerfMeasure) {
			...
			for (auto & m_perfTimer : m_perfTimers) {
				if (!m_perfTimer.IsDone) {
					isAllDone = false;

					int queryDone = 0;
					glGetQueryObjectiv(m_perfTimer.Object, GL_QUERY_RESULT_AVAILABLE, &queryDone);

					if (queryDone) {
						glGetQueryObjectui64v(m_perfTimer.Object, GL_QUERY_RESULT, &m_perfTimer.LastTime);
						m_perfTimer.IsDone = true;
					}
				}
				totalTime += m_perfTimer.LastTime;
			}
		...
		for (int i = 0; i < m_items.size(); i++) {
			...
			if (it->Type == PipelineItem::ItemType::ShaderPass) {
				pipe::ShaderPass* data = static_cast<pipe::ShaderPass*>(it->Data);

				if (!data->Active || data->Items.empty() || data->RTCount == 0)
					continue;
				...
				for (int i = 0; i < MAX_RENDER_TEXTURES; i++) {
					...
					if (rt != m_rtColor) {
						...
						for (unsigned int j : previousTexture)
							if (j == rt) {
								usedPreviously = true;
								break;
							}
				...
				glViewport(0, 0, (int)rtSize.x, (int)rtSize.y);
				...
				for (int j = 0; j < data->Items.size(); j++) {
					...
					if (item->Type == PipelineItem::ItemType::Geometry || item->Type == PipelineItem::ItemType::Model || item->Type == PipelineItem::ItemType::VertexBuffer || item->Type == PipelineItem::ItemType::PluginItem) {
						...
						for (auto & itemVarValue : itemVarValues)
							if (itemVarValue.Item == item)
								itemVarValue.Variable->Data = itemVarValue.NewValue->Data;

						if (isDebug) {
							float r = (float)(debugID & 0x000000FF) / 255.0f;
							float g = (float)((debugID & 0x0000FF00) >> 8) / 255.0f;
							float b = (float)((debugID & 0x00FF0000) >> 16) / 255.0f;
					...
					if (item->Type == PipelineItem::ItemType::Geometry) {
						pipe::GeometryItem* geoData = static_cast<pipe::GeometryItem*>(item->Data);
						...
						pipe::Model* objData = static_cast<pipe::Model*>(item->Data);
						...
						pipe::VertexBuffer* vbData = static_cast<pipe::VertexBuffer*>(item->Data);
						ed::BufferObject* bobj = static_cast<ed::BufferObject*>(vbData->Buffer);
						...
						pipe::RenderState* state = static_cast<pipe::RenderState*>(item->Data);
						...
						pipe::PluginItemData* pldata = static_cast<pipe::PluginItemData*>(item->Data);
					...
					if (item->Type == PipelineItem::ItemType::Geometry || item->Type == PipelineItem::ItemType::Model || item->Type == PipelineItem::ItemType::VertexBuffer || item->Type == PipelineItem::ItemType::PluginItem)
						for (auto & itemVarValue : itemVarValues)
							if (itemVarValue.Item == item)
								itemVarValue.Variable->Data = itemVarValue.OldValue;
				}
				...
				pipe::ComputePass* data = static_cast<pipe::ComputePass*>(it->Data);
				...
				for (int j = (int)ubos.size(); j < cMax; j++)
				...
				pipe::PluginItemData* pldata = static_cast<pipe::PluginItemData*>(it->Data);
		...
		if (m_pickAwaiting) {
			...
			if (m_pickHandle != nullptr)
				m_pickHandle(m_pick.empty() ? nullptr : m_pick[m_pick.size() - 1]);
			m_pickAwaiting = false;
		}
	...
	int RenderEngine::DebugVertexPick(PipelineItem* vertexData, PipelineItem* vertexItem, glm::vec2 r, int group)
	{
		if (vertexData->Type == PipelineItem::ItemType::ShaderPass) {
			pipe::ShaderPass* vertexPass = static_cast<pipe::ShaderPass*>(vertexData->Data);
			...
			// update viewport value
			glViewport(0, 0, (GLsizei)rtSize.x, (GLsizei)rtSize.y);
			...
			for (int j = 0; j < vertexPass->Items.size(); j++) {
				...
				if (item->Type == PipelineItem::ItemType::Geometry) {
					...
					for (const auto& mesh : objData->Data->Meshes) {
						int vertexStart = (group >= 0) * group;
						int maxVertexCount = (group < 0 ? (int)mesh.Indices.size() : (vertexStart + DEBUG_PRIMITIVE_GROUP * 3));
						int vertexCount = (group < 0 ? DEBUG_PRIMITIVE_GROUP : 1) * 3;

						maxVertexCount = std::min<int>(maxVertexCount, (int)mesh.Indices.size());
			...
			pipe::PluginItemData* plData = static_cast<pipe::PluginItemData*>(vertexData->Data);
	...
	int RenderEngine::DebugInstancePick(PipelineItem* vertexData, PipelineItem* vertexItem, glm::vec2 r, int group)
	{
		if (vertexItem->Type == PipelineItem::ItemType::Geometry) {
			pipe::GeometryItem* geoData = static_cast<pipe::GeometryItem*>(vertexItem->Data);
			if (!geoData->Instanced)
				return 0;
		} else if (vertexItem->Type == PipelineItem::ItemType::Model) {
			pipe::Model* objData = static_cast<pipe::Model*>(vertexItem->Data);
			if (!objData->Instanced)
				return 0;
		} else if (vertexItem->Type == PipelineItem::ItemType::VertexBuffer) {
			pipe::VertexBuffer* objData = static_cast<pipe::VertexBuffer*>(vertexItem->Data);
			if (!objData->Instanced)
				return 0;
		} else
			return 0;
		...
		if (vertexData->Type == PipelineItem::ItemType::ShaderPass) {
			...
			for (int j = 0; j < vertexPass->Items.size(); j++) {
				...
				if (item->Type == PipelineItem::ItemType::Geometry) {
					...
					for (const auto& mesh : objData->Data->Meshes) {
						int vertexCount = (int)mesh.Indices.size();
						int iStart = (group >= 0) * group;
	...
	void RenderEngine::m_pickItem(PipelineItem* item, bool multiPick)
	{
		...
		if (item->Type == PipelineItem::ItemType::Geometry) {
			...
			if (geo->Type == pipe::GeometryItem::GeometryType::Cube) {
				...
				if (glm::intersectRaySphere(vec3Origin, glm::normalize(vec3Dir), glm::vec3(0), r, myDist) ) {
					// Intersection found, myDist contains the valid distance
					// Proceed with using myDist
				};
