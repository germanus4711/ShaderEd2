...
namespace ed {
	...
	GizmoObject::GizmoObject()
			...
			, m_mode(0)
			, m_lastDepth(0)
			, m_model(std::vector<eng::Model::Mesh>(), "", glm::vec3(0.0f), glm::vec3(0.0f))
	...
	int GizmoObject::Click(int x, int y, int vw, int vh)
	{
		...
		if (m_axisSelected != -1) {
			...
			if (m_mode == 2) {
				m_clickDegrees = (float)glm::degrees(atan2(x - (vw / 2), y - (vh / 2)));
	...
	{
		// don't handle the rotation controls here as we handle that in the HandleMouseMove method
		if (m_axisSelected == -1 || m_mode == 2)
			return false;
		...
		if (m_mode == 0) {
			...
			if (int snap = Settings::Instance().Preview.GizmoSnapTranslation; snap <= 0)
			...
			if (int snap = Settings::Instance().Preview.GizmoSnapScale; snap <= 0)
	...
	void GizmoObject::Render() const
	{
		...
		glUniformMatrix4fv((GLint)m_uMatWorldLoc, 1, GL_FALSE, glm::value_ptr(matWorld));
		glUniformMatrix4fv((GLint)m_uMatVPLoc, 1, GL_FALSE, glm::value_ptr(m_proj * m_view));
		...
		for (auto& obj : objects) {
			...
			glUniform3fv((GLint)m_uColorLoc, 1, glm::value_ptr(clr));
		...
		if (m_axisSelected != -1 && m_mode == 2 && Settings::Instance().Preview.GizmoRotationUI) {
			...
			glUniformMatrix4fv((GLint)m_uMatWVPLoc, 1, GL_FALSE, glm::value_ptr(uMatWVP));
	...
	int GizmoObject::m_getBasicAxisSelection(int mx, int my, int vw, int vh, float& depth)
	{
		...
		float mouseX = (float)mx / ((float)vw * 0.5f) - 1.0f;
		float mouseY = (float)my / ((float)vh * 0.5f) - 1.0f;
		...
		if (m_mode == 0 || m_mode == 1) {
			...
			if (axisRet != -1) {
				glm::vec3 planeOrigin = *m_trans;
				glm::vec4 planeNormal = glm::vec4(glm::normalize(planeOrigin - glm::vec3(SystemVariableManager::Instance().GetCamera()->GetPosition())), 0);
				auto intersect_rp = glm::intersectRayPlane(glm::vec4(rayOrigin, 1), glm::vec4(rayDir, 0), glm::vec4(planeOrigin, 1), planeNormal, depth);
			}
		}
