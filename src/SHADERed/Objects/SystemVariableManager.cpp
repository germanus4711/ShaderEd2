...
namespace ed {
	...
	void SystemVariableManager::Update(ed::ShaderVariable* var, void* item)
	{
		...
		if (var->System != ed::SystemShaderVariable::None) {
			...
			if (!isLastFrame) {
				...
				case SystemShaderVariable::None: break;
				case SystemShaderVariable::Count: break;
				}
			} else {
				glm::mat4 rawMatrix;
				switch (var->System) {
	...
	ed::SystemShaderVariable SystemVariableManager::GetTypeFromName(const std::string& name)
	{
		...
		if (vname.find("frame") != std::string::npos || vname.find("index") != std::string::npos)
			return SystemShaderVariable::FrameIndex;
		if (vname.find("size") != std::string::npos || vname.find("window") != std::string::npos || vname.find("viewport") != std::string::npos || vname.find("resolution") != std::string::npos || vname.find("res") != std::string::npos)
			return SystemShaderVariable::ViewportSize;
		if (vname.find("mouse") != std::string::npos || vname == "mpos")
			return SystemShaderVariable::MousePosition;
		if (vname == "view" || vname == "matview" || vname == "matv" || vname == "mview")
			return SystemShaderVariable::View;
		if (vname == "proj" || vname == "matproj" || vname == "matp" || vname == "mproj" || vname == "projection" || vname == "matprojection" || vname == "mprojection")
			return SystemShaderVariable::Projection;
		if (vname == "matvp" || vname == "matviewproj" || vname == "matviewprojection" || vname == "viewprojection" || vname == "viewproj" || vname == "mvp")
			return SystemShaderVariable::ViewProjection;
		if (vname.find("ortho") != std::string::npos)
			return SystemShaderVariable::Orthographic;
		if (vname.find("geo") != std::string::npos || vname.find("model") != std::string::npos)
			return SystemShaderVariable::GeometryTransform;
		if (vname.find("picked") != std::string::npos)
			return SystemShaderVariable::IsPicked;
		if (vname.find("pick") != std::string::npos)
			return SystemShaderVariable::PickPosition;
		if (vname.find("issaving") != std::string::npos)
			return SystemShaderVariable::IsSavingToFile;
		if (vname.find("cam") != std::string::npos)
			return SystemShaderVariable::CameraPosition3;
		if (vname.find("keys") != std::string::npos || vname.find("wasd") != std::string::npos)
