...
namespace ed {
	...
	void DebugInformation::m_setupVM(std::vector<unsigned int>& spv)
	{
		...
		// link GLSL.std.450
		spvm_state_set_extension(m_vm, (char*)"GLSL.std.450", m_vmGLSL);
	}
	...
	void DebugInformation::m_setThreadID(spvm_state_t worker, int x, int y, int z, int numGroupsX, int numGroupsY, int numGroupsZ)
	{
		...
		int localIndex = (z % worker->owner->local_size_z) * worker->owner->local_size_x * worker->owner->local_size_z + (y % worker->owner->local_size_y) * worker->owner->local_size_x + (x % worker->owner->local_size_x);
			...
			for (int i = 0; i < varCount; i++) {
				...
				for (int c = 0; c < cm; c++) {
					for (int r = 0; r < rm; r++) {
						...
						case ShaderVariable::ValueType::Boolean2: break;
						case ShaderVariable::ValueType::Boolean3: break;
						case ShaderVariable::ValueType::Boolean4: break;
						case ShaderVariable::ValueType::Integer2: break;
						case ShaderVariable::ValueType::Integer3: break;
						case ShaderVariable::ValueType::Integer4: break;
						case ShaderVariable::ValueType::Float2: break;
						case ShaderVariable::ValueType::Float3: break;
						case ShaderVariable::ValueType::Float4: break;
						case ShaderVariable::ValueType::Float2x2: break;
						case ShaderVariable::ValueType::Float3x3: break;
						case ShaderVariable::ValueType::Float4x4: break;
						case ShaderVariable::ValueType::Count: break;
						}
					}
				}
			}
				...
				if (pointerInfo->storage_class == SpvStorageClassUniformConstant && !isBufferBlock) {
					...
					if (type_info->value_type == spvm_value_type_sampled_image || type_info->value_type == spvm_value_type_image) {
						...
						if ((wrongBind || textureID == 0) && !pluginUsesCustomTextures) {
							...
							// Cast to spvm_image_t for storage
							spvm_image_t img = &imgDataStruct->base;

							// Store in your slot
							slot->members[0].value.image = img;
							...
							if (pluginUsesCustomTextures)
								m_imageToTextureID[img] = pluginCustomTexture;
							else
								m_imageToTextureID[img] = textureID;
	...
	spvm_result_t DebugInformation::Immediate(const std::string& entry, spvm_result_t& outType)
	{
		...
		spvm_result_t cbufferSource = spvm_state_get_result_with_value(m_vm, (char*)"");
		spvm_result_t cbufferTarget = spvm_state_get_result_with_value(m_vmImmediate, (char*)"");
		...
		spvm_word fnImmediate = spvm_state_get_result_location(m_vmImmediate, (char*)"$$_shadered_immediate");
	...
	glm::vec4 DebugInformation::ExecuteVertexShader()
	{
		...
		if (fnMain == 0) {
			fnMain = spvm_state_get_result_location(m_vm, (char*)"main");
			if (fnMain == 0)
				return glm::vec4(0.0f);
		}
	...
	void DebugInformation::PrepareDebugger()
	{
		...
		if (spvm_program_get_language(m_vm->owner) == SpvSourceLanguageHLSL) {
	...
	void DebugInformation::UpdateVectorWatchValue(size_t index)
	{
		...
		if (exprVal != nullptr && resType != nullptr) {
			...
			if (resType->value_type == spvm_value_type_int || resType->value_type == spvm_value_type_float || resType->value_type == spvm_value_type_vector) {
