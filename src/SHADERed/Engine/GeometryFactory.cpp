...
namespace ed::eng {
	...
	void generateFace(GLfloat* verts, const float radius, const float sx, const float sy, const int x, const int y)
	{
		float phi = static_cast<float>(y) * sy;
		float theta = static_cast<float>(x) * sx;
		// clang-format off
			GLfloat pt1[18] = {
		...
		phi = static_cast<float>(y) * sy;
		theta = static_cast<float>(x + 1) * sx;
		// clang-format off
			GLfloat pt2[18] = {
				(radius * sin(phi) * cos(theta)),
				(radius * sin(phi) * sin(theta)),
		...
		phi = static_cast<float>(y + 1) * sy;
		theta = static_cast<float>(x) * sx;
		// clang-format off
			GLfloat pt3[18] = {
				(radius * sin(phi) * cos(theta)),
				(radius * sin(phi) * sin(theta)),
	...
	void calcBinormalAndTangents(GLfloat* verts, GLuint vertCount)
	{
		...
		for (GLuint i = 0; i < vertCount; i += 3) {
			...
			auto v0 = glm::vec3(verts[j0 + 0], verts[j0 + 1], verts[j0 + 2]);
			auto v1 = glm::vec3(verts[j1 + 0], verts[j1 + 1], verts[j1 + 2]);
			auto v2 = glm::vec3(verts[j2 + 0], verts[j2 + 1], verts[j2 + 2]);

			// Shortcuts for normals
			auto n0 = glm::vec3(verts[j0 + 3], verts[j0 + 4], verts[j0 + 5]);
			auto n1 = glm::vec3(verts[j1 + 3], verts[j1 + 4], verts[j1 + 5]);
			auto n2 = glm::vec3(verts[j2 + 3], verts[j2 + 4], verts[j2 + 5]);

			// Shortcuts for UVs
			auto uv0 = glm::vec2(verts[j0 + 6], verts[j0 + 7]);
			auto uv1 = glm::vec2(verts[j1 + 6], verts[j1 + 7]);
			auto uv2 = glm::vec2(verts[j2 + 6], verts[j2 + 7]);
	...
	unsigned int GeometryFactory::CreateCube(unsigned int& vbo, const float sx, const float sy, const float sz, const std::vector<InputLayoutItem>& inp)
	{
		const float halfX = sx / 2.0f;
		const float halfY = sy / 2.0f;
		const float halfZ = sz / 2.0f;
	...
	{
		constexpr int numPoints = 32 * 3;
		constexpr int numSegs = numPoints / 3;
		...
		for (int i = 0; i < numSegs; i++) {
			...
			const float xVal1 = sin(step * static_cast<float>(i));
			const float yVal1 = cos(step * static_cast<float>(i));
			const float xVal2 = sin(step * static_cast<float>(i + 1));
			const float yVal2 = cos(step * static_cast<float>(i + 1));
	...
	{
		constexpr size_t stackCount = 20;
		constexpr size_t sliceCount = 20;

		constexpr size_t count = sliceCount * stackCount * 6;
		GLfloat sphereData[count * 18];

		constexpr float stepY = glm::pi<float>() / stackCount;
		constexpr float stepX = glm::two_pi<float>() / sliceCount;

		for (int i = 0; i < stackCount; i++) {
			// float phi = i * stepY;
			for (int j = 0; j < sliceCount; j++) {
				// float theta = j * stepX;
				const size_t index = (i * sliceCount + j) * 6;
	...
	{
		const float rightOffs = s / tan(glm::radians(30.0f));
		// clang-format off
			GLfloat triData[] = {
