...
namespace ed::eng {
	...
	void Model::Mesh::m_setup()
	{
		...
		glBufferData(GL_ARRAY_BUFFER, static_cast<long>(Vertices.size() * sizeof(Vertex)), &Vertices[0],
			GL_STATIC_DRAW);
		...
		// vertex positions
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Model::Mesh::Vertex), static_cast<void*>(nullptr));
		glEnableVertexAttribArray(0);

		// vertex normals
		glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Model::Mesh::Vertex), reinterpret_cast<void*>(offsetof(Vertex, Normal)));
		glEnableVertexAttribArray(1);

		// vertex texture coords
		glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(Model::Mesh::Vertex), reinterpret_cast<void*>(offsetof(Vertex, TexCoords)));
		glEnableVertexAttribArray(2);

		glBindVertexArray(0);
	}
	void Model::Mesh::Draw(const bool instanced, const int iCount) const
	{
		// draw mesh
		glBindVertexArray(VAO);

		if (instanced)
			glDrawElementsInstanced(GL_TRIANGLES, static_cast<int>(Indices.size()), GL_UNSIGNED_INT, nullptr, iCount);
		else
	...
	bool Model::LoadFromFile(const std::string& path)
	{
		...
		{
			Logger::Get().Log("Assimp has detected an error \"" + std::string(importer.GetErrorString()) + "\"", true);
			return false;
		}
	...
	std::vector<std::string> Model::GetMeshNames() const
	{
		...
		for (const auto& mesh : Meshes)
			ret.push_back(mesh.Name);
		return ret;
	}
	void Model::Draw(bool inst, int iCount) const
	{
		for (const auto & Mesh : Meshes)
			Mesh.Draw(inst, iCount);
	}
	void Model::Draw(const std::string& mesh) const
	{
		for (const auto & Mesh : Meshes)
			if (Mesh.Name == mesh)
				Mesh.Draw();
	}
	void Model::m_processNode(const aiNode* node, const aiScene* scene)
	{
	...
	Model::Mesh Model::m_processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		return Mesh(mesh->mName.data, vertices, indices, textures);
	}
}
