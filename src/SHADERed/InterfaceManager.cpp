...
namespace ed {
	...
	{
		const uint8_t* pxData = getRawPixel(rt, data, x, y, width);
		return { static_cast<float>(pxData[0]) / 255.0f, static_cast<float>(pxData[1]) / 255.0f, static_cast<float>(pxData[2]) / 255.0f, static_cast<float>(pxData[3]) / 255.0f };
	}
	uint32_t getPixelID(GLuint rt, uint8_t* data, int x, int y, int width)
	{
		uint8_t* pxData = getRawPixel(rt, data, x, y, width);
		return (static_cast<uint32_t>(pxData[0]) << 0) | (static_cast<uint32_t>(pxData[1]) << 8) | (static_cast<uint32_t>(pxData[2]) << 16) | (static_cast<uint32_t>(pxData[3]) << 24);
	}
	...
	void InterfaceManager::DebugClick(glm::vec2 r)
	{
		...
		int x = static_cast<int>(r.x) * previewSize.x;
		int y = static_cast<int>(r.y) * previewSize.y;
		...
		auto mainPixelData = new uint8_t[maxRTSize.x * maxRTSize.y * 4]; // copy gpu data to cpu
		...
		// rt pixel colors
		for (auto obj : objs) {
			if (obj->Type == ObjectType::RenderTexture) {
				GLuint tex = obj->Texture;
				glm::ivec2 rtSize = Objects.GetRenderTextureSize(obj);

				pixelColors[tex] = getPixelColor(tex, mainPixelData, static_cast<int>(r.x) * rtSize.x, static_cast<int>(r.y) * rtSize.y, rtSize.x);
			}
		}
		...
		for (const auto& k : pipelineItems) {
			...
			x = static_cast<int>(r.x) * rtSize.x;
			y = static_cast<int>(r.y) * rtSize.y;
			...
			if (k.second.second->Type == ed::PipelineItem::ItemType::Geometry) {
				auto* geoData = static_cast<pipe::GeometryItem*>(k.second.second->Data);

				objTopology = static_cast<int>(geoData->Topology);
				...
				instanceBuffer = static_cast<BufferObject*>(geoData->InstanceBuffer);
			} else if (k.second.second->Type == ed::PipelineItem::ItemType::VertexBuffer) {
				auto* bufData = static_cast<pipe::VertexBuffer*>(k.second.second->Data);

				objTopology = static_cast<int>(bufData->Topology);
				...
				auto objData = static_cast<pipe::Model*>(k.second.second->Data);
				instanceBuffer = static_cast<BufferObject*>(objData->InstanceBuffer);
				...
				objTopology = static_cast<int>(objData->Owner->PipelineItem_GetTopology(objData->Type, objData->PluginData));
			...
			if (k.second.first->Type == PipelineItem::ItemType::ShaderPass) {
				auto* pass = static_cast<pipe::ShaderPass*>(k.second.first->Data);
	...
	void InterfaceManager::FetchPixel(PixelInformation& pixel)
	{
		...
		if (pixel.InstanceBuffer) {
			auto* buf = (BufferObject*)pixel.InstanceBuffer;
		...
		if (pixel.Object->Type == PipelineItem::ItemType::VertexBuffer) {
			auto* vertBufferItem = static_cast<pipe::VertexBuffer*>(pixel.Object->Data);

			if (auto* buf = static_cast<BufferObject*>(vertBufferItem->Buffer)) {
				ObjectManagerItem* bufOwner = Objects.GetByBufferID(buf->ID);
	...
	void InterfaceManager::m_fetchVertices(PixelInformation& pixel)
	{
		if (pixel.Object->Type == PipelineItem::ItemType::Geometry) {
			pipe::GeometryItem::GeometryType geoType = static_cast<pipe::GeometryItem*>(pixel.Object->Data)->Type;
			const GLuint vbo = static_cast<pipe::GeometryItem*>(pixel.Object->Data)->VBO;
			...
			auto mdl = static_cast<pipe::Model*>(pixel.Object->Data);

			int curOffset = 0;
			for (const auto& mesh : mdl->Data->Meshes) {
				if (pixel.VertexID >= curOffset + mesh.Indices.size()) {
					curOffset += static_cast<int>(mesh.Indices.size());
					continue;
				}
			...
			const pipe::VertexBuffer* vBuffer = static_cast<pipe::VertexBuffer*>(pixel.Object->Data);
			auto* bufData = static_cast<ed::BufferObject*>(vBuffer->Buffer);

			const std::vector<ShaderVariable::ValueType> tData = Objects.ParseBufferFormat(bufData->ViewFormat);

			int stride = 0;
			for (const auto& dataEl : tData)
				stride += ShaderVariable::GetSize(dataEl, true);

			auto* bufPtr = (GLfloat*)malloc(pixel.VertexCount * stride);
			...
			const auto* pdata = static_cast<pipe::PluginItemData*>(pixel.Object->Data);

			const GLuint vbo = pdata->Owner->PipelineItem_GetVBO(pdata->Type, pdata->PluginData);
			const GLuint vboStride = pdata->Owner->PipelineItem_GetVBOStride(pdata->Type, pdata->PluginData);
			const int inpLayoutSize = pdata->Owner->PipelineItem_GetInputLayoutSize(pdata->Type, pdata->PluginData);
			...
			for (int i = 0; i < pixel.VertexCount; i++) {
				...
				for (int j = 0; j < inpLayoutSize; j++) {
					...
					const int valSize = static_cast<int>(InputLayoutItem::GetValueSize(static_cast<InputLayoutValue>(layItem.Value)));
	...
	bool InterfaceManager::m_canDebug()
	{
		...
		for (const auto& i : Pipeline.GetList()) {
			if (i->Type == PipelineItem::ItemType::ShaderPass) {
				auto* pass = static_cast<pipe::ShaderPass*>(i->Data);
				int langID = -1;
