...
SpvSourceLanguage spvm_program_get_language(spvm_program_t prog) {
    if (prog && prog->file_count > 0)
        return prog->files[0].language;
    return SpvSourceLanguageUnknown; // or some appropriate default
}
